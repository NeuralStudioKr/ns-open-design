...
class OdDaemonClient:
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=self.timeout_seconds) as client:
            response = await client.post(
                f"{self.base_url}/api/projects/{od_project_id}/export/html",
                headers={
                    **self._headers(accept="text/html,*/*", identity=identity),
        ...
        if response.status_code >= 400:
            logger.warning(
                "[od-daemon] POST /export/html failed project=%s status=%s body=%s",
                od_project_id,
    ...
    async def _request_export_ticket(
        ...
        *,
        ...
        offload_enabled = body.get("offloadEnabled", False)
        ...
        if not isinstance(size, int) or size < 0:
            raise BadGatewayError("od_daemon_invalid_export_ticket")
        if delivery_mode not in {"stream", "redirect"}:
            raise BadGatewayError("od_daemon_invalid_export_ticket")
