...
- **Token-safe content-swap:** preserve the selected template's slide classes, layout roles, surface colors, decorative wrappers, border/shadow/card treatment, and SVG motif language from the kit + scaffold map. Replace only visible content (headings, paragraphs, bullets, charts/tables/images) to match the user brief. Do NOT dump or rewrite a full example.html.
...
- **Fixed 1920×1080 canvas is non-negotiable.** Every \`<section class="slide">\` MUST inline \`width:1920px;height:1080px;box-sizing:border-box;overflow:hidden\`. Do not use \`100vw\`/\`100vh\`, \`min-height:100vh\`, or scroll-snap presenter plumbing — the preview panel is a scaled canvas, not a fullscreen viewport.
