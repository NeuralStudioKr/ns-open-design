...
3. ${COMPACT_FIRST_FILL_SLIDE_COUNT_GUIDANCE} Write one filled \`<section class="slide">\` per requested slide. Each slide needs a heading AND a \`<p>\`/\`<li>\` sentence — title-only slides fail persist. If an on-disk deck is attached, keep at least that slide count (do not rewrite an 8-slide deck as 3). If a Selected deck template is active, match its visual kit (palette/fonts/density) with inline styles or one short body \`<style>\` after slide 1 — design system is brand context only and must not override the template look; do not merely describe the template.
...
**Forbidden on deck turns:** outlines, plans, TodoWrite, \`[读取 template.html]\`, SLOT comments, a second artifact, stopping after \`<head>\`, announcing a brand-new draft on an edit turn, parroting the user brief as slide titles/body, emitting the same heading/paragraph/badge twice in a row, or announcing completion without the requested slide count (${COMPACT_FIRST_FILL_SLIDE_COUNT_GUIDANCE}).
...
**Forbidden:** "바로 만들어 드리겠습니다" / "I'll make it" promise-only replies, question-form, outlines, plans, TodoWrite, \`[读取 template.html]\`, SLOT comments, a second artifact, stopping after \`<head>\`, parroting the user brief as slide titles/body, emitting the same heading/paragraph/badge twice in a row, announcing completion without the requested slide count (${COMPACT_FIRST_FILL_SLIDE_COUNT_GUIDANCE}), or repeating the same layout/background/composition on every slide. Preserve the Selected deck template look (design system is secondary brand context only) and vary layouts per the compact inline layout vocabulary.`;
...
- **Contrast:** light templates need light \`html\`/\`body\`/\`.slide\` + dark ink; dark templates need dark surface + light ink. Never emit dark-on-dark or cream-slides-on-dark-shell. A slide that paints no background inherits the kit paper — light copy (\`#93c5fd\`, \`#e0e7ff\`) or a \`rgba(255,255,255,0.05)\` glass card on that paper reads as an empty slide.
