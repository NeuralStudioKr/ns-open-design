...
- **Slide surface binding is authoritative — bind BOTH the outer document AND every slide.** Read \`### Slide surface\` in the kit and use its exact \`background\` / \`color\` hex on **both** \`html\` / \`body\` **and** every \`<section class="slide">\`. Painting only the slides while leaving \`body\` at a dark app-shell default produces cream slides floating on a dark shell — same failure as dark-on-dark. Never substitute an ink / border / stroke token (e.g. \`#2D2D2D\`, \`#232323\`, \`#1E1E1C\`) for the slide background — those are the outline color the template uses on borders and headings, not a surface color. **Dark-on-dark, light-on-light, and cream-slides-on-dark-shell are all failed deliverables**; verify contrast: light kit surface → dark ink text; dark kit surface → light ink text.
...
- **Contrast is non-negotiable — and \`html\` / \`body\` must inherit the same surface as the slides.** If the template name / summary implies a light surface (pastel, cream, ivory, warm paper, editorial white), paint both \`html\` / \`body\` AND every \`<section class="slide">\` with a light background + dark ink text. If it implies a dark surface (terminal, midnight, noir), paint both with a dark background + light ink. Cream slides floating on a dark app-shell body is the same failure as dark-on-dark; never emit that shape.
