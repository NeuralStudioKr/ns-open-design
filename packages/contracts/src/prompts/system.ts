...
- **Slide surface binding is authoritative.** Read \`### Slide surface\` in the kit and use its exact \`background\` / \`color\` hex on every \`<section class="slide">\` (or the body). Never substitute an ink / border / stroke token (e.g. \`#2D2D2D\`, \`#232323\`, \`#1E1E1C\`) for the slide background — those are the outline color the template uses on borders and headings, not a surface color. **Dark-on-dark or light-on-light slides are a failed deliverable**; verify contrast: light kit surface → dark ink text; dark kit surface → light ink text.
...
- **Contrast is non-negotiable.** If the template name / summary implies a light surface (pastel, cream, ivory, warm paper, editorial white), the slide background must be light and text ink must be dark; if it implies a dark surface (terminal, midnight, noir), background dark + text light. Never emit dark-on-dark or light-on-light slides.
