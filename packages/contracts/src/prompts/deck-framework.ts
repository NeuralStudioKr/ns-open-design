...
**Three-column** — headline + 3 points (use flex, keep copy short). Column count = card count: a titled N-item row must emit N cards (flex children or equal 1fr / 1.0fr / minmax(0,1fr) / 33% / 33vw tracks) — never a 3-track grid with only two cards, and never an empty card shell or placeholder card shell (제목/내용/.../TBD/N/A/n.a./준비중/해당없음/작성예정/empty/blank/dummy/예시/tobefilled/PILLAR 03/PILLAR III/No. 3/KEY 3/Phase 3/③) to hold a leftover track. Peer cards share the row: no px/%/vw width or min-width, max-width, or flex: 0 0 on each card. Class flex rows (.cards { display:flex }) must also share the canvas. Never wrap a card in another card:
