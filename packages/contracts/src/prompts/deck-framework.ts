...
**Motif (after title):** REQUIRE 1–2 kit Motif CSS/deco classes AFTER title/lead when the kit lists Decorations / Motif classes. Motif \`<svg>\` is NOT required this turn — official Motif CSS/SVG is merged after save. Finish 3 closed slides this turn. FORBIDDEN: Motif \`<svg>\` this turn; Motif \`<svg>\` before cover title; multi-KB \`<svg><style>\` dumps; Motif geometry from another template family; generic circles/stars as substitutes; inventing Capsule coral pills when the kit Motif is petals/blobs/pins/pixel/scanlines; inventing tiny corner flower SVGs / emoji daisies / 12–48px butter circles; omitting all kit Motif CSS cues when Decorations are listed.
