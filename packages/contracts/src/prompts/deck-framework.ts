...
**Three-column** — headline + 3 points (use flex, keep copy short). Column count = card count: a titled N-item row must emit N cards (flex children or equal 1fr tracks) — never a 3-track grid with only two cards, and never an empty card shell or placeholder card shell (제목/내용/.../TBD/N/A/준비중/해당없음/PILLAR 03) to hold a leftover track. Peer cards share the row: no px/% width or min-width, max-width, or flex: 0 0 on each card. Class flex rows (.cards { display:flex }) must also share the canvas. Never wrap a card in another card:
...
**Three-column** — headline + 3 short points (flex/grid); kit card borders/shadows when the kit uses chunky cards. Column count = card count: never leave an empty third track or an empty/placeholder card shell (TBD/N/A/준비중/해당없음/PILLAR 03). Peer cards share the row: no px/% width or min-width, max-width, or flex: 0 0 on each card. Class flex rows must also share the canvas. Never wrap a card in another card.
