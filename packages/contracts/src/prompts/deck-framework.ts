...
Required wireframe only (structural — **do not** copy literally; use varied layouts below). This sample is **three slides on purpose**: persist rejects a one-slide closed cover unless the user asked for 1 page. Never close \`</html></artifact>\` after a single section:
...
**Motif (after title):** use kit Motif HTML snippets / Motif CSS / at most one capped sprite. A named motif family from the title/vocabulary (flowers/daisies/capsule/pins/pixels/scanlines/terminal chrome/etc.) must visibly appear on the cover and at least one body slide. Finish a closed deck this turn — light Motif beats a truncated Motif dump. FORBIDDEN: Motif \`<svg>\` before cover title; multi-KB \`<svg><style>\` dumps; Motif geometry from another template family; generic circles/stars as substitutes; inventing Capsule coral pills when the kit Motif is petals/blobs/pins/pixel/scanlines; inventing tiny corner flower SVGs / emoji daisies / 12–48px butter circles when the kit has Daisy Motif sprites (official Motif is merged after save); ignoring kit Motif snippet aspect/border-radius.
