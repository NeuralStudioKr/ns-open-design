...
**Three-column** — headline + 3 points (use flex, keep copy short). Column count = card count: a titled N-item row must emit N cards (flex children or equal 1fr tracks) — never a 3-track grid with only two cards, and never an empty card shell to hold a leftover track:
...
**Three-column** — headline + 3 short points (flex/grid); kit card borders/shadows when the kit uses chunky cards. Column count = card count: never leave an empty third track or an empty card shell.
