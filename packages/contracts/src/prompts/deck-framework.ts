...
**Three-column** — headline + 3 points (use flex, keep copy short). Column count = card count: a titled N-item row must emit N cards (flex children or equal 1fr / 1.0fr / 0.33fr / minmax(0,1fr) / 33% / 33vw tracks or 33vh/vmin/dvw/cqmin/dvmin/vi) — never a 3-track grid with only two cards, and never an empty card shell or placeholder card shell (제목/내용/.../TBD/N/A/n.a./준비중/해당없음/작성예정/empty/blank/dummy/예시/tobefilled/soon/later/대기/lorem ipsum/placeholder/자료없음/임시/fake/xxx/null/pass/foo/FIXME/PILLAR 03/PILLAR 00/PILLAR 10/PILLAR III/No. 3/KEY 3/Phase 3/Module 3/Group 3/Chapter 3/③/０/C/3번/(3)/D/첫째/셋째/E/여섯째) to hold a leftover track. Peer cards share the row: no px/%/vw width or min-width, vh/vmin, max-width, or flex: 0 0 on each card. Class flex rows (.cards { display:flex }) must also share the canvas. Never wrap a card in another card:
