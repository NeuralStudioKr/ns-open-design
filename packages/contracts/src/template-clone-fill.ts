...
/**
 * 루프499 — Grove fingerprint (forest `--c-bg` + Playfair/Jost + grove-*).
 * Shares `slide--cover`/`slide--end` + `slide-chrome`/`compare-panel` with
 * Studio — resolve Grove first and deny from Studio.
 */
export function officialLookIsGrove(html: string): boolean {
  const source = String(html ?? '');
  if (!source.trim()) return false;
  if (officialLookIsCreativeMode(source)) return false;
  ...
  if (!groveChrome) return false;
  const fonts = /Playfair Display/i.test(hay) || /\bJost\b/i.test(hay);
  const forest = /--c-bg\s*:\s*#192b1b/i.test(hay) || /#192b1b/i.test(hay);
...
function preservedCoverIdentityAttrs(attrs: string): string {
  const parts: string[] = [];
  const screen = /\bdata-screen-label\s*=\s*(['"])([^'"]*)\1/i.exec(attrs);
  if (screen) parts.push(`data-screen-label=${screen[1]}${screen[2]}${screen[1]}`);
  const dataSlide = /\bdata-slide\s*=\s*(['"])([^'"]*)\1/i.exec(attrs);
  if (dataSlide) parts.push(`data-slide=${dataSlide[1]}${dataSlide[2]}${dataSlide[1]}`);
...
export function restyleForeignIbMagazineCover(html: string): string {
  ...
  if (capsule) {
    ...
    return (
      `${dest.slice(0, first.start)}<${first.tag} class="slide slide-1"${preservedCoverIdentityAttrs(first.attrs)} `
  ...
  if (eightBit) {
    ...
    const inner =
      ...
      + `</div>`;
    return (
      `${dest.slice(0, first.start)}<${first.tag} class="slide bg-grid scanlines grain crt-glow"${preservedCoverIdentityAttrs(first.attrs)} `
      + `style="width:1920px;height:1080px;box-sizing:border-box;overflow:visible;`
  ...
  return (
    `${dest.slice(0, first.start)}<${first.tag} class="slide s-cover"${preservedCoverIdentityAttrs(first.attrs)} ` +
    `style="width:1920px;height:1080px;box-sizing:border-box;overflow:visible;` +
...
/** 루프499 — Grove: no kit vertical writing; cover/end .h1 stay horizontal (sidebar uses rotate). */
const GROVE_POSTER_LAYOUT_CSS = [
  '.slide--cover .h1,.slide--cover .kicker,.slide--cover .lead,.slide--end .h1,.slide--end .kicker,.slide--end .lead{writing-mode:horizontal-tb!important;-webkit-writing-mode:horizontal-tb!important}',
...
const GROVE_COVER_KIT_SLOT_RE =
  /\b(?:grove-sidebar|grove-num|kicker|rule|h1|lead|label|muted|slide-chrome|slide-foot|nav-hint|pagenum|data-od-official-motif-html)\b/i;
...
function resolveOfficialPosterKit(html: string): OfficialPosterKitPlan | null {
  ...
  if (officialLookIsGrove(dest)) {
    return {
      kind: 'grove',
      coverHostRe: /\bslide--cover\b/i,
      closingHostRe: /\bslide--end\b/i,
      coverSlotRe: GROVE_COVER_KIT_SLOT_RE,
      colophonSlotRe: GROVE_CLOSING_KIT_SLOT_RE,
...
/** Full inner replace so leftover `<br/>2026` / long demo titles do not survive. */
function replaceCobaltTitleHeading(html: string, text: string): string {
  const headingRe = /<(h[1-3])\b([^>]*\b(?:title|ttl)\b[^>]*)>([\s\S]*?)<\/\1>/i;
  if (!headingRe.test(html)) return html;
  return html.replace(
    headingRe,
...
/**
 * Hangul Cobalt persist leftover: Field Office chrome may already be gone
 * (루프451) while index/table/stmt/quote/chapter still carry catalog letters.
 * Fill those slots from the brief, then wipe leftover phrases. Do not invent
 * KPIs — `.vbig` digits stay. Official English example.html is a no-op.
 */
export function healCobaltLeftoverCatalogCopy(
  html: string,
  brief?: string | null,
): string {
  const dest = String(html ?? '');
  if (!dest.trim() || !officialLookIsCobaltGrid(dest)) return dest;
  if (!/[가-힣]/.test(visibleDeckCopy(dest))) return dest;
  let out = scrubCobaltFieldOfficeDemoSlots(dest);
  const spans = listHealSlideHostSpans(out);
  if (spans.length === 0) return stripLeftoverCatalogDemoPhrases(out);
  ...
  for (let i = spans.length - 1; i >= 0; i -= 1) {
    const span = spans[i]!;
    if (!/\bs-(?:cover|manifesto|index|chapter|data|quote|table|colophon)\b/i.test(span.attrs)) {
      continue;
    }
    const body = out.slice(span.bodyStart, span.bodyEnd);
    if (
      !looksLikeLeftoverTemplateDemoDeck(body)
      && !/The index, in six entries|Trend ledger|Reader response|A note from the studio|Chapter one|Index(?:<br\s*\/?>|\s)+2026/i.test(body)
    ) {
      continue;
    }
    const slide = outline[i] ?? outline[Math.min(i, outline.length - 1)];
    const title = slide?.title || harvested[i] || harvested[0] || '슬라이드';
    const fillLines = templateCloneSlideFillLines(slide ?? { title });
    const input = {
      title,
      lead: slide?.lead ?? '',
      bodyText: slide?.body ?? '',
    ...
    let nextBody = replaceCobaltTitleHeading(body, title);
    if (/\bs-cover\b/i.test(span.attrs)) {
      nextBody = fillCobaltCoverChrome(nextBody, input);
    }
    if (/\b(?:s-quote|s-manifesto)\b/i.test(span.attrs)) {
      nextBody = fillBiennaleQuoteSlots(nextBody, input);
    }
    if (/\bs-index\b/i.test(span.attrs)) {
      nextBody = fillCobaltRowPeers(nextBody, input, 'index');
    }
    if (/\bs-table\b/i.test(span.attrs)) {
      nextBody = fillCobaltRowPeers(nextBody, input, 'table');
    }
    ...
    if (/\bs-data\b/i.test(span.attrs)) {
      const lines = biennaleFillLines(input, 2);
      nextBody = replaceFirstExactClassText(nextBody, 'h', title);
      nextBody = replaceFirstExactClassText(nextBody, 'lab-tag', input.kicker || input.lead || title);
      nextBody = replaceClassTextBySequence(nextBody, 'lab2', lines.map((line) => line.title));
      nextBody = replaceClassTextBySequence(nextBody, 'desc', lines.map((line) => line.body || line.title));
    }
    if (/\bs-colophon\b/i.test(span.attrs)) {
      nextBody = fillCobaltColophonFooter(nextBody, input);
    }
    if (nextBody === body) continue;
    out = `${out.slice(0, span.bodyStart)}${nextBody}${out.slice(span.bodyEnd)}`;
  }
...
export function buildTemplateClonedDeckHtml(
  ...
  if (cleanedSlides.length > 0) {
    ...
    // 루프549 — Overflow safety for shell reuse. Non-unique-role kits (product-
    // launch / pitch / most Zhangzara) reuse shells to reach the requested slide
    // count. That is intentional (Mat/Pink Script/Vellum ship 7–9 shells but the
    // deterministic quality gate expects 10 slides). What we DO block is
    // *identical outline titles* being placed on reused shells — the 2026-09-17
    // user report had slide 3 and slide 9 both carrying `주제의 쓰임과 근거`,
    // producing near-duplicate Fit slides. `dedupeIdenticalOutlineTitles` keeps
    // the first occurrence and rewrites later duplicates to indexed fallback
    // labels so the reused shell gets distinct copy.
  ...
  // 루프549 — MiniMax token-loop 반복(예: `다음 단계 다음 단계`) 축약.
...
/**
 * 루프549 — Apply `collapseAdjacentDuplicatedPhraseInText` inside heading /
...
function slideTitleParrotsBriefFragment(title: string, brief?: string | null): boolean {
  ...
  // 루프549 — MiniMax는 두 번째 슬라이드에 종종 `{brief} 2`, `{brief} · 2`,
  // `{brief} 시리즈 2`처럼 브리프에 단순 번호/카운터만 붙인 제목을 뱉는다.
  // 예: brief="Teamver 소개" → title="Teamver 소개 2" (product-launch example.
  // html 슬라이드 2의 `Halo v2` 슬롯). instruction-copy 브리프가 아니어도
  // 브리프 원문을 그대로 파롯한 뒤 번호만 더한 shape면 실패 제목으로 본다.
...
/**
 * 루프549 — MiniMax often emits Korean placeholder-shaped titles when the brief
 * is thin, e.g. `주제가 해결하는 문제`, `주제의 쓰임과 근거`, `주제를 쓰는 순서`,
 * `주제 다음 단계`. `sanitizeTemplateCloneDeckTitle` treats these as valid because
 * they are grammatical strings, so `rewriteInstructionParrotingSlideTitles` leaves
 * them untouched. Result: user-facing decks with "주제" (Korean word for "topic")
 * plastered across every heading (2026-09-17 사용자 리포트).
 *
 * Trigger only when the *bare* Korean placeholder word ("주제", "제목", "내용",
 * "본문") starts the title AND is followed by a particle/verb/noun that shows it
 * was written as a scaffold, not as a real subject reference. Real titles never
 * lead with the standalone word "주제 " — users say "AI 주제", "환경 주제", etc.
 */
...
/**
 * 루프549 — MiniMax token-loop repetition. When the model gets stuck it emits
 * doubled phrases: `다음 단계 다음 단계`, `쓰는 순서를 쓰는 순서`,
 ...
 * Case 2: `X<particle> X` — first occurrence carries a Korean particle; second
 *         is bare. Preserve the first (with particle) and drop the tail.
 *
...
function collapseAdjacentDuplicatedPhraseInText(text: string): string {
  ...
  for (let pass = 0; pass < 3; pass += 1) {
    ...
    out = out.replace(
      ...
      (match, phrase: string) => {
        // Preserve the first phrase + its particle. Match includes both copies;
        // slice off the second copy so grammar is preserved.
...
function rewriteInstructionParrotingSlideTitles(
  ...
  return slides.map((slide, index) => {
    // 루프549 — 먼저 MiniMax token-loop 반복(`X를 X`, `X X`)을 축약한다.
...
/**
 * 루프549 — Rewrite any later slide whose title exactly matches an earlier
 ...
 * `rewriteInstructionParrotingSlideTitles` funnels multiple placeholder-shaped
 * slides through the same fallback shape), the reused shell renders two
 * near-identical Fit/Body slides in the same deck (2026-09-17 사용자 리포트:
 * slide 3 · slide 9 both `주제의 쓰임과 근거`). Keeping the first occurrence
 * and appending ` · N` to later duplicates preserves the outline order while
 * making each slide's headline distinct — the shell can still be reused, but
 * downstream copy synthesis sees a unique title and produces distinct body
 * copy per slot.
 */
function dedupeIdenticalOutlineTitles(
  ...
  return slides.map((slide, index) => {
    ...
    const suffix = `${cover} · ${index + 1}`;
    const next: TemplateCloneSlideContent = { ...slide, title: suffix };
    return next;
  });
}
