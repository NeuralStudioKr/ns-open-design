...
describe("chat leak / persist probe round 717 (set124 combo px+ch)", () => {
  it("px+ch + FOO ❆ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(5px + 1ch);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/5px \+ 1ch[^>]*\binfo-card\b|info-card[^>]*5px \+ 1ch/i);
