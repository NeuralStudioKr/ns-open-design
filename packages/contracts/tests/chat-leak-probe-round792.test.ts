...
describe("chat leak / persist probe round 792 (set139 combo px minus)", () => {
  it("px minus + FOO ☏ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(10px + 5px - 2px);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/10px \+ 5px - 2px[^>]*\binfo-card\b|info-card[^>]*10px \+ 5px - 2px/i);
