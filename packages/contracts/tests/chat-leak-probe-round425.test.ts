...
describe("chat leak / persist probe round 425 (set70 combo)", () => {
  it("leading-dot + FOO ⇝ + invent-frame off", () => {
    const kit = [
    ...
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/\.75rem[^>]*\binfo-card\b|info-card[^>]*\.75rem/i);
