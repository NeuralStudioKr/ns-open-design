...
describe("chat leak / persist probe round 642 (set109 combo pt+vh)", () => {
  it("pt+vh + FOO ⊞ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(4pt + 1vh);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/4pt \+ 1vh[^>]*\binfo-card\b|info-card[^>]*4pt \+ 1vh/i);
