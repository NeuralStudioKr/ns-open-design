...
describe("chat leak / persist probe round 794 (set140 combo paren rem+px)", () => {
  it("paren rem+px + FOO ♯ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc((0.5rem + 8px) - 2px);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/\(0\.5rem \+ 8px\) - 2px[^>]*\binfo-card\b|info-card[^>]*\(0\.5rem \+ 8px\) - 2px/i);
