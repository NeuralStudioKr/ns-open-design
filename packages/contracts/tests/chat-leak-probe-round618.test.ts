...
describe("chat leak / persist probe round 618 (set99 combo lh+ic)", () => {
  it("lh+ic + FOO ⬢ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(1lh + 0.5ic);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/1lh \+ 0\.5ic[^>]*\binfo-card\b|info-card[^>]*1lh \+ 0\.5ic/i);
