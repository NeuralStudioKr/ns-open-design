...
describe("chat leak / persist probe round 643 (set110 combo pt+cq)", () => {
  it("pt+cqw + FOO ⋆ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(4pt + 1cqw);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/4pt \+ 1cqw[^>]*\binfo-card\b|info-card[^>]*4pt \+ 1cqw/i);
