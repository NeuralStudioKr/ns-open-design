...
describe("chat leak / persist probe round 695 (set120 combo vb+pt)", () => {
  it("vb+pt + FOO ❅ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(1vb + 4pt);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/1vb \+ 4pt[^>]*\binfo-card\b|info-card[^>]*1vb \+ 4pt/i);
