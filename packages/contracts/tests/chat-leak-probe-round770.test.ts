...
describe("chat leak / persist probe round 770 (set135 combo vh+cq+px)", () => {
  it("vh+cqw+px + FOO ✏ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(0.5vh + 0.5cqw + 4px);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/0\.5vh \+ 0\.5cqw \+ 4px[^>]*\binfo-card\b|info-card[^>]*0\.5vh \+ 0\.5cqw \+ 4px/i);
