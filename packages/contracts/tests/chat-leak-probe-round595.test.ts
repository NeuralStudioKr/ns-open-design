...
describe("chat leak / persist probe round 595 (set102 combo ‡)", () => {
  it("cqi+cqb + FOO ‡ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(1cqi + 1cqb);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/1cqi \+ 1cqb[^>]*\binfo-card\b|info-card[^>]*1cqi \+ 1cqb/i);
