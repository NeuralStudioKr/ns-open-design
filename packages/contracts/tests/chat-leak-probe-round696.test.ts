...
describe("chat leak / persist probe round 696 (set123 closure)", () => {
  it("rem/line/ch/vb+print + FOO + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(0.5rem + 4pt);border:1px solid navy">a</span>',
      '<span style="padding:calc(0.5lh + 4pt);border:1px solid teal">b</span>',
      '<span style="padding:calc(1ch + 4pt);border:1px solid purple">c</span>',
      '<p style="padding:calc(0.2rem + 2pt);border:1px solid gray">thin</p>',
      "</section>",
    ].join("");
    const bound = bindFakeOutlineCardsToOfficialKit(kit);
    expect(bound).toMatch(/0\.5rem \+ 4pt[^>]*\binfo-card\b|info-card[^>]*0\.5rem \+ 4pt/i);
    expect(bound).toMatch(/0\.5lh \+ 4pt[^>]*\binfo-card\b|info-card[^>]*0\.5lh \+ 4pt/i);
    expect(bound).toMatch(/1ch \+ 4pt[^>]*\binfo-card\b|info-card[^>]*1ch \+ 4pt/i);
