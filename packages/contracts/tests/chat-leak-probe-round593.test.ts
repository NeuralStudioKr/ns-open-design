...
describe("chat leak / persist probe round 593 (set100 combo ic+FOO)", () => {
  it("ic+ric + FOO ※ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(0.5ic + 0.5ric);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/0\.5ic \+ 0\.5ric[^>]*\binfo-card\b|info-card[^>]*0\.5ic \+ 0\.5ric/i);
