...
describe("chat leak / persist probe round 594 (set100 combo print+FOO)", () => {
  it("pt+mm + FOO † + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(6pt + 2mm);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/6pt \+ 2mm[^>]*\binfo-card\b|info-card[^>]*6pt \+ 2mm/i);
