...
describe("chat leak / persist probe round 571 (set98 closure)", () => {
  it("mixed line-box + fontVp + FOO geometry + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(1lh + 1ex);border:1px solid navy">a</span>',
      '<span style="padding:calc(1vb + 1vi);border:1px solid teal">b</span>',
      '<p style="padding:calc(0.5lh + 0.5ex);border:1px solid gray">thin</p>',
      "</section>",
    ].join("");
    const bound = bindFakeOutlineCardsToOfficialKit(kit);
    expect(bound).toMatch(/1lh \+ 1ex[^>]*\binfo-card\b|info-card[^>]*1lh \+ 1ex/i);
    expect(bound).toMatch(/1vb \+ 1vi[^>]*\binfo-card\b|info-card[^>]*1vb \+ 1vi/i);
