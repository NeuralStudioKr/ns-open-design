...
describe("chat leak / persist probe round 239 (invent-frame keep + FOO)", () => {
  it("keeps invent-frame off flow and drops FOO chrome", () => {
    const html = [
      '<section class="slide" style="-ms-flex:1;-moz-columns:2;box-shadow:0 0 0 1px red;border-top:1px solid navy;background-color:snow;width:1920px;height:1080px">',
      "<div>x</div>",
      "</section>",
    ].join("");
    const flow = pinDeckSlidesToFixedCanvas(html).match(/<div\b[^>]*\bdata-od-slide-flow\b[^>]*>/i)?.[0] ?? "";
    expect(flow).toMatch(/-ms-flex:\s*1/i);
    expect(flow).toMatch(/-moz-columns:\s*2/i);
