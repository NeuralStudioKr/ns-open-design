...
describe("chat leak / persist probe round 510 (set87 combo)", () => {
  it("rem+em + FOO ○ + invent-frame off", () => {
    const kit = [
    ...
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/0\.5rem \+ 0\.25em[^>]*\binfo-card\b|info-card[^>]*0\.5rem \+ 0\.25em/i);
