...
describe("chat leak / persist probe round 570 (set97 combo □)", () => {
  it("svb+svi + FOO □ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(1svb + 1svi);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/1svb \+ 1svi[^>]*\binfo-card\b|info-card[^>]*1svb \+ 1svi/i);
