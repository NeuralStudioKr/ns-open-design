...
describe("chat leak / persist probe round 694 (set120 combo ch+pt)", () => {
  it("ch+pt + FOO ❃ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(1ch + 4pt);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/1ch \+ 4pt[^>]*\binfo-card\b|info-card[^>]*1ch \+ 4pt/i);
