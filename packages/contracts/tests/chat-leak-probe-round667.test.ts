...
describe("chat leak / persist probe round 667 (set114 combo rem+vh)", () => {
  it("rem+vh + FOO ✙ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(0.4rem + 1vh);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/0\.4rem \+ 1vh[^>]*\binfo-card\b|info-card[^>]*0\.4rem \+ 1vh/i);
