...
describe("chat leak / persist probe round 592 (set99 combo cq+FOO)", () => {
  it("cqw+cqh + FOO ✶ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(1cqw + 1cqh);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/1cqw \+ 1cqh[^>]*\binfo-card\b|info-card[^>]*1cqw \+ 1cqh/i);
