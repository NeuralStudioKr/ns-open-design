...
describe("chat leak / persist probe round 745 (set130 combo px+Q)", () => {
  it("px+Q + FOO ♔ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(5px + 10Q);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/5px \+ 10Q[^>]*\binfo-card\b|info-card[^>]*5px \+ 10Q/i);
