...
describe("chat leak / persist probe round 744 (set130 combo px+in)", () => {
  it("px+in + FOO ⚀ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(5px + 0.1in);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/5px \+ 0\.1in[^>]*\binfo-card\b|info-card[^>]*5px \+ 0\.1in/i);
