...
describe("chat leak / persist probe round 846 (set153 closure)", () => {
  it("mul chain + nested calc + FOO + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(7px * 2 / 1);border:1px solid navy">a</span>',
      '<span style="padding:calc(calc(3px + 4px) * 2);border:1px solid teal">b</span>',
      '<span style="padding:calc(2 * 7px * 1);border:1px solid purple">c</span>',
      '<p style="padding:calc(5px * 2 / 1);border:1px solid gray">thin</p>',
      "</section>",
    ].join("");
    const bound = bindFakeOutlineCardsToOfficialKit(kit);
    expect(bound).toMatch(/7px \* 2 \/ 1[^>]*\binfo-card\b|info-card[^>]*7px \* 2 \/ 1/i);
    expect(bound).toMatch(/calc\(3px \+ 4px\) \* 2[^>]*\binfo-card\b|info-card[^>]*calc\(3px \+ 4px\) \* 2/i);
    expect(bound).toMatch(/2 \* 7px \* 1[^>]*\binfo-card\b|info-card[^>]*2 \* 7px \* 1/i);
