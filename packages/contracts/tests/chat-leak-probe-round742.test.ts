...
describe("chat leak / persist probe round 742 (set129 combo px+pt)", () => {
  it("px+pt + FOO ⚝ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(5px + 6pt);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/5px \+ 6pt[^>]*\binfo-card\b|info-card[^>]*5px \+ 6pt/i);
