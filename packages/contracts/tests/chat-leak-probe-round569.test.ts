...
describe("chat leak / persist probe round 569 (set96 combo ✪)", () => {
  it("cap+ex + FOO ✪ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(1cap + 1ex);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/1cap \+ 1ex[^>]*\binfo-card\b|info-card[^>]*1cap \+ 1ex/i);
