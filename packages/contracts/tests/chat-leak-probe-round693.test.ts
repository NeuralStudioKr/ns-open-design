...
describe("chat leak / persist probe round 693 (set120 combo lh+pt)", () => {
  it("lh+pt + FOO ❁ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(0.5lh + 4pt);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/0\.5lh \+ 4pt[^>]*\binfo-card\b|info-card[^>]*0\.5lh \+ 4pt/i);
