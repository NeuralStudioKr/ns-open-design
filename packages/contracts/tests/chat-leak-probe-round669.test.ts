...
describe("chat leak / persist probe round 669 (set115 combo lh+vh)", () => {
  it("lh+vh + FOO ✠ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(1lh + 1vh);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/1lh \+ 1vh[^>]*\binfo-card\b|info-card[^>]*1lh \+ 1vh/i);
