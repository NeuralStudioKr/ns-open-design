...
describe("chat leak / persist probe round 795 (set140 combo paren triple)", () => {
  it("paren triple + FOO ♪ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc((0.3rem + 4px + 0.5vh) - 1px);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/\(0\.3rem \+ 4px \+ 0\.5vh\) - 1px[^>]*\binfo-card\b|info-card[^>]*\(0\.3rem \+ 4px \+ 0\.5vh\) - 1px/i);
