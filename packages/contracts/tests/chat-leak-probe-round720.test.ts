...
describe("chat leak / persist probe round 720 (set125 combo px+cq)", () => {
  it("px+cqw + FOO ⚛ + invent-frame off", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:calc(3px + 1cqw);border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/3px \+ 1cqw[^>]*\binfo-card\b|info-card[^>]*3px \+ 1cqw/i);
