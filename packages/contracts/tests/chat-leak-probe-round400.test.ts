...
describe("chat leak / persist probe round 400 (set65 combo)", () => {
  it("Q padding kit + invent-frame off + FOO »", () => {
    const kit = [
      '<style data-od-official-look-css>.info-card{border:1px solid var(--border)}</style>',
      '<section class="slide" style="width:1920px;height:1080px">',
      '<span style="padding:8Q;border:1px solid navy">ok</span>',
      "</section>",
    ].join("");
    expect(bindFakeOutlineCardsToOfficialKit(kit)).toMatch(/8Q[^>]*\binfo-card\b|info-card[^>]*8Q/i);
