...
describe('FileViewer revision list retry', () => {
  it('retries revision list fetch failures without showing a conflict toast', async () => {
    ...
    await waitFor(() => expect(getListCallCount()).toBeGreaterThan(2), { timeout: 4_000 });
    ...
    expect(screen.getByTestId('file-viewer-undo').hasAttribute('disabled')).toBe(false);
  });
});
