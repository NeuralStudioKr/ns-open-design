...
describe('FileViewer revision disk sync recovery', () => {
  it('keeps undo available when background disk sync fails after cache-fast undo', async () => {
    const initialSource = heroSource();
    const {
      fetchMock,
      getPersistedSource,
      setRestoreShouldFail,
    ...
    await waitFor(() => {
      expect(screen.getByRole('alert').textContent).toMatch(/saving to disk failed|디스크에 저장하지 못했습니다/);
    }, { timeout: 8_000 });
    expect(screen.getByTestId('file-viewer-redo').hasAttribute('disabled')).toBe(false);
    expect(screen.getByTestId('file-viewer-undo').getAttribute('data-tooltip') ?? '').not.toMatch(
