...
describe('FileViewer revision history restore', () => {
  it('restores an older revision without showing a conflict toast', async () => {
    ...
    act(() => {
      panelState.props?.onApplyPatch(
        { kind: 'set-style', id: 'hero', styles: { color: '#22c55e' } },
        'Style: Hero green',
      );
    });
    await waitFor(() => expect(getRevisions().length).toBeGreaterThanOrEqual(3));
    await waitFor(() => expect(getPersistedSource()).toContain('rgb(34, 197, 94)'));
    ...
    const baselineRevision = getRevisions().find((revision) => revision.id === 'rev-baseline');
    expect(baselineRevision).toBeTruthy();

    const restoreButton = screen.getByTestId(`file-revision-restore-${baselineRevision!.sequence}`);
    await act(async () => {
      fireEvent.click(restoreButton);
    });

    await waitFor(() => {
      expect(getPersistedSource()).toBe(baselineRevision!.content);
    });
