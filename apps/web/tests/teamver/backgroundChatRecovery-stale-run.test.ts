...
describe("backgroundChatRecovery stale run helpers", () => {
  ...
  it("polls and forces failure for stale API-mode assistant rows without runId", () => {
    // Loop411/508 — force-fail stays above deck SSE idle (6m).
    expect(TEAMVER_STALE_API_RUN_RECONCILE_MS).toBe(6 * 60 * 1000);
    expect(TEAMVER_STALE_API_RUN_FORCE_FAIL_MS).toBe(7 * 60 * 1000);
    const now = Date.now();
