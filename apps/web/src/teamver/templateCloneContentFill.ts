...
export function templateCloneContentFillHardRules(): string[] {
  return [
    ...
    '- Motif vocabulary OVERRIDE: Motif floor is REQUIRED (not optional polish). Title-first always: AFTER cover `<h1>`/`<h2>` + lead, place at least ONE kit Motif seed on the cover — kit Motif HTML snippets / Decorations CSS Motif classes / capped Motif sprites (whatever the kit ships). When Motif sprites lists a kit SVG (Daisy flowers etc.), that ONE capped sprite MUST appear on the cover (nest inside `.deco` / `.deco-daisy-*` when listed) — kit sprites may be ~2KB and are exempt from the ~800-char Motif-budget. Motif hosts MUST be large corner Motifs (≥150px / official `.deco-daisy-tl|br` placement) — FORBIDDEN: 24–40px sticker flowers that leave cream covers looking Motif-less. If token budget is tight, finish ≥3 closed slides with EMPTY `.deco-daisy-tl` / `.deco-daisy-br` shells (no SVG body) — persist injects official Motif SVG after save. Prefer 1–2 Motif elements after title when scaffold lists `deco=` — finish a closed deck this turn. FORBIDDEN: inventing generic CSS circles, tiny corner dots, or Capsule coral pills when the kit Motif is petals/flowers/blobs/pins/pixel/scanlines; empty `.deco` shells without child SVG when budget allows; Motif `<svg>` before title copy. If you already started an SVG-before-title dump, abandon it and restart with `<h1>`.',
    ...
    '- Prefer finishing a closed `</artifact>` with ≥3 slides this turn over perfect motif fidelity, but Motif floor is still required: at least one kit Motif seed OR empty `.deco-daisy-*` shell on the cover. A complete compact Daisy-lookalike beats a truncated SVG/CSS shell.',
