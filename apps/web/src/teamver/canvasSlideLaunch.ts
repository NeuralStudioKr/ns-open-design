...
function selectedSlideTemplatePriorityInstruction(title: string): string {
  return [
    ...
    "Follow scaffold map layout roles (do not flatten every slide into the same cover). Use compact template motif/deco cues from the kit after title/body copy starts; small complete snippets are OK, but never paste huge SVG/style payloads or any Motif block that risks a hang. Do not invent ellipse daisy SVGs or emoji ornaments.",
