...
export function wrapSelectedDeckTemplateSkillBody(
  ...
  return [
    ...
    '',
    'When a **Template scaffold (CONTENT-SWAP BASE)** is included below: start FROM',
    'that HTML. Replace only visible text/content for the user brief. Keep the',
    'scaffold CSS, classes, Motif SVGs, `.deco` wrappers, borders, and fonts',
    'verbatim. Do NOT reinvent a “similar vibe” with a new palette.',
    '',
    'When only a Template visual kit is present (no scaffold): bind kit tokens /',
    'Motif sprites with inline styles or one short body `<style>` — still never',
    'fall back to Neutral Modern / OD skeleton terracotta.',
    '',
    ...
    'match the template thumbnail. Keep source structure/headings/callouts/tables;',
    'only the LOOK follows the template.',
    '',
    ...
    'symbols. Do not invent ellipse daisy SVGs when Motif sprites are in the scaffold.',
    ...
    'inline styles (or one short body `<style>`), including kit Motif sprites / `.deco` density.',
    ...
    'If the preview uses SVG daisies, stars, stickers, badges, frames,',
    'or chunky CSS borders, reproduce those with the provided SVG/CSS/HTML shapes.',
    '',
