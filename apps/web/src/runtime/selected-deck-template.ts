...
export function wrapSelectedDeckTemplateSkillBody(
  ...
  return [
    ...
    '`<!doctype html><html lang="ko"><body><section class="slide" ...>`.',
    'Do not emit `<head>`, `<title>`, meta tags, a full example.html stylesheet, or a large SVG block before the first filled slide.',
    'Prefer CSS shapes / `.deco` / chunky borders for decorative density. Motif SVG is optional: at most one short snippet AFTER visible title/body copy has started; never open Motif `<svg>` before cover copy; skip huge SVG `<style>` payloads or any Motif paste that risks a hang.',
    'On first Clone content-fill: ZERO Motif `<svg>` — use compact CSS shapes / `.deco` cues in kit palette hex so the deck can close this turn.',
