...
export function wrapSelectedDeckTemplateSkillBody(
  ...
  return [
    ...
    'Prefer kit Motif vocabulary for decorative density: Motif HTML snippets / Decorations CSS Motif classes / capped Motif sprites AFTER title/lead (whatever the kit ships). When scaffold `deco=` lists classes, emit ≥2 Motif elements. Never invent generic CSS circles or Capsule coral pills when the kit Motif is petals/blobs/pins/pixel/scanlines. Never open Motif `<svg>` before cover copy; skip huge SVG `<style>` dumps.',
