...
export function wrapSelectedDeckTemplateSkillBody(
  ...
  return [
    ...
    'Prefer kit Motif vocabulary for decorative density: Motif HTML snippets / Decorations CSS Motif classes / capped small Motif snippets AFTER title/lead (whatever the kit ships). Use 1–2 lightweight template-identifying Motif/deco cues per slide when scaffold lists `deco=` or the kit exposes motif classes — finish a closed deck this turn. Never invent generic CSS circles or Capsule coral pills when the kit Motif is petals/blobs/pins/pixel/scanlines. Never open Motif `<svg>` before cover copy; skip huge SVG `<style>` dumps.',
    'Reuse capped Layout CSS + scaffold roles when present — do not flatten every slide into one centered flex title column.',
    'On first Clone content-fill: title-first, then kit Motif vocabulary + capped Layout. Never open Motif `<svg>` before cover title; never omit kit identity or invent generic circles when the kit has Motif vocabulary.',
