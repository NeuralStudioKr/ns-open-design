...
describe('file-revisions durable postgres authority', () => {
  ...
  it('hydrates when head matches but sqlite has stale extra rows', async () => {
    const db = openDb();
    db.prepare(`
      INSERT INTO file_revisions (
        ...
        ('rev-1', 'proj-1', 'deck.html', NULL, 1, 1, 10, 'import', 'Baseline', NULL, NULL),
        ('rev-2', 'proj-1', 'deck.html', 'rev-1', 2, 2, 12, 'manual_edit', 'Edit', NULL, NULL),
        ('rev-stale', 'proj-1', 'deck.html', 'rev-2', 3, 3, 12, 'manual_edit', 'Stale', NULL, NULL)
    `).run();
    const pgRows = [
      {
        id: 'rev-1',
