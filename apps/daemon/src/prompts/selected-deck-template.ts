...
export function wrapSelectedDeckTemplateSkillBody(
  ...
  return [
    ...
    '',
    'Token-safe content-swap: daemon already seeded the selected `example.html`',
    'look into deck.html. Treat that sample as the DOM/style baseline: preserve',
    'surface/classes/layout roles/motif geometry while replacing visible content.',
    'Use the **Template visual kit** (+ scaffold map) as the compact prompt form.',
    'Do NOT dump or rewrite full example.html (input/output token risk).',
    '',
    ...
    'Prefer kit Motif vocabulary for decorative density: Motif HTML snippets / Decorations CSS Motif classes / capped Motif sprites AFTER title/lead (whatever the kit ships). Prefer 1–2 visible Motif elements after title when scaffold lists `deco=` — finish a closed deck this turn. Empty `.deco` shells do not count. Never invent generic CSS circles or Capsule coral pills when the kit Motif is petals/blobs/pins/pixel/scanlines. Never open Motif `<svg>` before cover copy; skip huge SVG `<style>` dumps.',
    'Reuse capped Layout CSS + scaffold roles when present — do not flatten every slide into one centered flex title column.',
    'On first Clone content-fill: title-first, then kit Motif vocabulary + capped Layout. Never open Motif `<svg>` before cover title; never invent generic circles when the kit has Motif vocabulary.',
    ...
    'not a surgical edit of the seeded deck.html. On that fill turn emit a JSON outline only',
    '({"title","slides":[{"title","body","roleHint"}]}); the host slot-fills the LOOK seed.',
    'Do NOT emit <!doctype / <section class="slide"> HTML on the first Clone content-fill turn.',
    '',
