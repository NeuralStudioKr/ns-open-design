...
export function wrapSelectedDeckTemplateSkillBody(
  ...
  return [
    ...
    'If Motif sprites are present, use at most one short snippet AFTER visible title/body copy has started; skip huge SVG `<style>` payloads.',
    'On first Clone content-fill: skip Motif `<svg>` entirely — use compact CSS shapes / `.deco` cues in kit palette hex so the deck can close this turn.',
