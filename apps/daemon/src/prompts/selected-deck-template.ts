...
export function wrapSelectedDeckTemplateSkillBody(
  ...
  return [
    ...
    'Prefer kit Motif vocabulary for decorative density: capped Motif sprites AFTER title/lead and/or Decorations CSS Motif classes listed in the kit (pills/petals/blobs/pins/`.deco-*`/pixel ornaments — whatever the kit ships). Never invent generic CSS circles when the kit has Motif CSS/sprites. Never open Motif `<svg>` before cover copy; skip huge SVG `<style>` dumps.',
