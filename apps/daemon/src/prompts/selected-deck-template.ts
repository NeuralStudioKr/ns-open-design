...
export function wrapSelectedDeckTemplateSkillBody(
  ...
  return [
    ...
    'Prefer kit Motif vocabulary for decorative density: Motif HTML snippets / Decorations CSS Motif classes / capped Motif sprites AFTER title/lead (whatever the kit ships). Prefer 1–2 Motif elements after title when scaffold lists `deco=` — finish a closed deck this turn. Never invent generic CSS circles or Capsule coral pills when the kit Motif is petals/blobs/pins/pixel/scanlines. Never open Motif `<svg>` before cover copy; skip huge SVG `<style>` dumps.',
