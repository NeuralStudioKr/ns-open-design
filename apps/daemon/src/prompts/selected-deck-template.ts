...
export function wrapSelectedDeckTemplateSkillBody(
  ...
  return [
    ...
    'Do not emit `<head>`, `<title>`, meta tags, or a full example.html stylesheet before the first filled slide.',
    'If Motif sprites are present AND this is NOT a first Clone content-fill turn, copy at most one provided SVG onto the cover AFTER the title text exists.',
    'On first Clone content-fill: skip Motif `<svg>` entirely — use CSS shapes in kit palette hex so the deck can close this turn.',
