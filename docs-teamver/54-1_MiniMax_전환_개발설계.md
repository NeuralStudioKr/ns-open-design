...
- MiniMax OpenAI-compatible SSE parser와 chunk normalization
- MiniMax `tool_calls` loop
- P0 tool `web_fetch` 연결
- MiniMax 응답의 hidden/thinking/tool markup streaming-safe sanitizer
- MiniMax deck-only artifact validation과 incomplete_output recovery
- staging smoke: 새 deck 생성, 기존 deck 수정, 댓글 scoped patch, 파일/Drive 첨부, URL 참조

### 26.4 검증 메모

2026-08-04 별도 worktree에서 `pnpm install` 후 다음을 확인했습니다.
...
1. daemon `/api/proxy/minimax/stream` route를 추가합니다.
2. MiniMax request builder에서 `max_tokens`를 제외합니다.
3. OpenAI-compatible SSE를 기존 FE `streamProxyEndpoint` 이벤트 형태로 변환합니다.
4. `web_fetch`만 우선 tool loop에 연결합니다.
5. streaming 중 `<think>`, `<tool>`, `<invoke>`, `<question>`, deliverable instruction이 채팅 저장/렌더에 노출되지 않도록 기존 sanitizer를 MiniMax route에도 강제합니다.
6. route 구현 뒤에야 `TEAMVER_DESIGN_DEFAULT_PROVIDER=minimax`를 staging에서 켭니다.
