...
### 5.5c BYOK proxy stream server hook (P1 · `mode=api` 유지)

**트리거:** `POST /api/proxy/{anthropic,openai,azure,google,ollama,senseaudio,aihubmix}/stream`
요청에 `projectId` 가 포함된 경우 (Teamver embed chat surface).

**코드:** `apps/daemon/src/storage/byok-proxy-materialization.ts` —
`attachByokProxyStreamMaterialization` → `beforeChatRun` (stream 시작 직전) /
`afterChatRun` (`res` finish·close).

| 단계 | 시점 | 동작 |
|------|------|------|
| ① proxy handler | validation 통과 후 SSE 시작 전 | `beforeChatRun` → sync-down |
| ② stream 중 | BYOK tools | scratch write |
| ③ proxy SSE 종료 | HTTP `finish`/`close` | `afterChatRun` → sync-up (run-start 이후 mtime 필터) |

**env:**
- `OD_BYOK_PROXY_MATERIALIZATION=1` (S3 mode default on, `0` disable).
- `OD_BYOK_PROXY_FAIL_ON_BEGIN=1` (default on). `beforeChatRun` sync-down
  실패 시 stream 을 502 로 종료 → FE 가 사용자에게 명확한 오류 노출.
  `=0` 으로 끄면 legacy "warn + stream without sync" 동작이 복원되며,
  이 모드에서는 [29](./29_BYOK_api_mode_vs_runs_아키텍처.md) 의
  데이터 손실 클래스가 재발한다 — production 에서 끄지 말 것.

**§5.5b 와 관계:** proxy finish 가 1차 sync-up, terminal message PUT 이 2차
(defense in depth). 상세: [29 BYOK api mode vs runs](./29_BYOK_api_mode_vs_runs_아키텍처.md).

**SIGTERM drain 안전성 (2026-06-29 리뷰 픽스):** BYOK proxy stream finalize 는
`wrapFinish` 가 아닌 `res.close`/`res.finish` 리스너로 동작하므로
이전에는 `drainAfterChatRun` 의 추적 set 에 포함되지 않았다 →
SIGTERM 중 sync-up upload 가 진행 중인 BYOK stream 은 daemon exit 으로
중단되어 scratch 가 S3 에 반영되지 못하는 race. 픽스로
`ProjectMaterializationRuntime.trackExternalAfterChatRun(promise)` 를 노출하고,
`byok-proxy-materialization.ts` finalize 가 그 헬퍼로 sync-up promise 를
drain set 에 등록한다. SIGTERM 은 이제 BYOK stream sync-up 까지 대기 후 exit.

### 5.5d BYOK proxy 스트림 취소 정책 (Stop vs page-exit)

**문제 정의:** embed BYOK 채팅의 SSE stream 을 도중에 끊을 수 있는 경로는
두 종류이고, daemon 에서 처리해야 할 의미가 다르다.

| 트리거 | FE 동작 | daemon 동작 | 이유 |
|--------|---------|-------------|------|
| **Stop 버튼** (`handleStop`) | `controller.abort(EXPLICIT_PROXY_STOP_REASON)` + 별도 `POST /api/proxy/abort { streamId }` (keepalive) | upstream `fetch` 의 `AbortController.abort()` 호출 → LLM API 도 즉시 종료 → `afterChatRun` (sync-up) | 사용자가 명시적으로 "그만" → 토큰·시간 낭비 방지 |
| 페이지 이탈 / 라우트 변경 / supersede | `controller.abort()` (reason 미지정) | **abort 하지 않음**. browser-side SSE consumer 만 detach; upstream 은 자연 완료까지 진행 → `afterChatRun` (sync-up) | 백그라운드 도구 작업(이미지 생성·S3 쓰기)이 진행 중일 수 있으므로 끊지 말 것 |

**코드:**
- daemon — `apps/daemon/src/byok-proxy-abort.ts` 의 `registerByokProxyStream` /
  `registerByokProxyAbortRoute` (`POST /api/proxy/abort`). `chat-routes.ts` 의
  모든 proxy stream handler 가 등록하고, upstream `fetch({ signal })` 에 전달.
- FE — `apps/web/src/providers/proxyAbort.ts` (`EXPLICIT_PROXY_STOP_REASON` +
  `requestProxyAbort(streamId)`), `apps/web/src/providers/api-proxy.ts`
  (`X-Stream-Id` 헤더 → signal listener), `apps/web/src/components/ProjectView.tsx`
  `handleStop` (모든 controller `.abort(EXPLICIT_PROXY_STOP_REASON)`).

**Stream ID 라이프사이클:**

```text
1. daemon: registerByokProxyStream() → randomUUID() → activeProxyStreams.set(streamId, …)
2. daemon: res.setHeader('X-Stream-Id', streamId)  ← SSE body 시작 전
3. FE: const streamId = resp.headers.get('X-Stream-Id')  ← stream consume 직전
4. FE: signal.addEventListener('abort', …)
5. (사용자 Stop) FE: signal.reason === EXPLICIT_PROXY_STOP_REASON
                    → fetch('/api/proxy/abort', { method:'POST', keepalive:true, body:{streamId} })
6. daemon: abortByokProxyStream(streamId) → AbortController.abort()
           → upstream LLM fetch reject → SSE close
7. daemon: res.once('finish'|'close') → finalize → afterChatRun → sync-up
8. daemon: registry 정리 — 연결 유지 시 res.finish 즉시. page-exit 로 res.close 가
           먼저 오면 upstream handler 의 res.end() 까지 entry 유지
           (fallback TTL 15분).
```

**2026-07-03 재진입 복구 보강:** page-exit `res.close` 때 active proxy
entry 를 즉시 제거하면 upstream 은 백그라운드에서 계속 돌지만 FE 재진입 시
`/api/proxy/active` 가 빈 배열이 되어 "작업 중" 상태와 파일 refresh 를 복원할
단서가 사라진다. 따라서 `activeProxyStreams` 는 `conversationId` /
`assistantMessageId` 를 함께 보관하고, browser response close 후에도 upstream
handler 가 `res.end()` 할 때까지 listable 하게 유지한다. FE 는 재진입 시 active
proxy 를 먼저 확인해 해당 assistant row 를 in-flight 로 보강하고, 완료 전까지
messages/files 를 polling 한다.

**Tenant scoping (defense in depth):** `streamId` 는 122-bit
cryptographically random UUID 이며 발급된 session 외부로 새지 않지만,
`POST /api/proxy/abort` 는 등록 entry 의 `workspaceId` 와 caller 의
`X-Workspace-Id` 가 일치할 때만 abort 한다. 불일치 시 `{ aborted: false }`
응답 (unknown streamId 와 동일) — 존재 여부 leak 방지.

**Registry 한계:** `activeProxyStreams` 는 daemon-process-local in-memory map.
다중 daemon 인스턴스 (HA scaling) 환경에서는 sticky session 없이는 abort 가
다른 instance 에 닿지 않을 수 있다 — embed BYOK 는 현재 single-instance
배포이며 multi-instance 전환 시 redis-pubsub 등으로 brokering 필요.
...
| `OD_BYOK_PROXY_MATERIALIZATION` | default **on** (S3) · `0` disable | embed BYOK proxy stream → before/afterChatRun sync ([29](./29_BYOK_api_mode_vs_runs_아키텍처.md)) |
...
| 2026-06-29 | §5.5c BYOK proxy stream server hook (P1) · `OD_BYOK_PROXY_MATERIALIZATION` |
...
| 2026-06-29 | §5.5c SIGTERM drain 안전성 — `trackExternalAfterChatRun` 으로 BYOK proxy finalize 까지 drain 대기 (production 출시 리뷰 픽스) |
| 2026-06-29 | §5.5d BYOK proxy 취소 정책 (Stop = upstream abort, page-exit = drain) · `POST /api/proxy/abort` tenant scoping · `X-Stream-Id` header |
| 2026-06-29 | `beforeChatRun` rollback race fix — concurrent run 의 active counter / floor entry 를 wipe 하지 않는 decrement-from-current 의미론 |
| 2026-07-03 | §5.5d BYOK page-exit 재진입 복구 보강 — active proxy entry 를 upstream end 까지 유지, `conversationId`/`assistantMessageId` 로 FE messages/files polling 복원 |
