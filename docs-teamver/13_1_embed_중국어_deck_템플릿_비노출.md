...
`TEAMVER_EMBED_HIDDEN_CHINESE_PRIMARY_DECK_TEMPLATE_IDS` (11개) — SSOT: `@open-design/contracts` `embed-chinese-deck-policy.ts`
...
비노출 목록 11개 `SKILL.md` frontmatter에 추가:
...
embed 필터는 **denylist + `contentLocale === 'zh-CN'`** 이중 조건 — 이후 템플릿은 메타만으로도 자동 비노출 가능.
...
1. staging embed Home 커뮤니티 — `Guizang`, `XHS`, `Hermes 사이버` 등 카드 미노출
2. Settings → Design templates — 동일 11개 미노출
3. standalone OD — 11개 정상 노출
