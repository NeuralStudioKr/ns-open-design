...
### Clone LOOK vs 완성 덱 (2026-08-18)

템플릿 Clone은 LOOK preview로 `deck.html`을 먼저 심는다. 카드 썸네일은 그 파일의 **첫 슬라이드 live HTML**이다 (플러그인 `preview.png` / example.html 갤러리 썸네일이 아님).

fill이 `deck.html`을 덮어쓴 뒤에도 path-only `htmlCoverCache`가 Clone 첫 슬라이드를 들고 있으면 홈/목록이 템플릿 기본 커버로 남는다. HTML 저장 성공 시 `clearProjectCoverCache`가 hint 캐시와 해당 프로젝트 srcDoc를 같이 버리고, 카드는 refetch한다. html-ppt `.slide{opacity:0}` 는 isolation 후 남은 커버를 `opacity:1`로 강제한다.
...
| 2026-08-18 | Clone LOOK `deck.html` 첫 슬라이드 srcDoc가 fill 이후에도 path-only 캐시에 남아 템플릿 기본 썸네일로 보이던 문제 — HTML write 시 cover+srcDoc bust, isolation 후 첫 슬라이드 visible 강제 |
