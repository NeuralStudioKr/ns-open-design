# 00-1 설계 — Manual Edit 요소 박스 드래그 리사이즈

**시리즈:** `docs-teamver/수동편집_드래그_리사이즈/`  
**문서 번호:** 00-1 (설계)  
**기획:** [00 기획](./00_기획.md)  
**구현 현황:** [00-2 구현현황](./00-2_구현현황.md)  
...
| 2026-07-30 | **시리즈 00으로 이관** + 상태머신·수식·좌표·자격·세션·수용기준 보강 |
...
| Committing | `flushManualEditStyleSave` 1회 (또는 동등 apply 경로). 성공 후 pause 해제 |
| Cancelling | pending을 stylesBefore로 되돌리고 preview, pause 해제, **POST 없음** |
...
- commit POST 실패: 기존 style flush 실패 복구 (`restoreManualEditPendingStyleAfterFailedFlush`). overlay는 draft 유지, 사용자 재시도 가능.
...
- 위치 이동 (layout DnD) — 새 00이 아닌 **새 시리즈**
...
- [ ] zoom 100% / 75% 에서 핸들이 카드 모서리에 붙음
- [ ] SE 드래그 → 소스 inline width/height px 갱신
- [ ] E만 드래그 → height 스타일 키 불필요하게 생기지 않음
- [ ] 이미지 코너: 기본 비율 유지, Shift로 해제
- [ ] 텍스트 박스 width 드래그, fontSize 불변
- [ ] `.slide` 선택 시 핸들 없음
- [ ] Esc 취소 시 디스크 불변
- [ ] 드래그 후 패널 숫자 일치, undo 1스텝에 리사이즈 전체 취소
- [ ] Draw 모드 전환 시 overlay 사라짐
- [ ] 인라인 텍스트 편집 중 핸들 비활성
...
| agent `transform` on element | 중 | rect는 getBoundingClientRect(이미 transform 반영). width style은 레이아웃 크기 — 이슈 시 패널 안내 |
...
6. [00-2](./00-2_구현현황.md) Phase 1 항목 전부 `[x]`.
...
| D5 | 문서 번호를 기능 폴더 로컬 00 시리즈로 재편 | 2026-07-30 |
