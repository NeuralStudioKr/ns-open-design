...
| **EC2** | `t3.large`, EIP | `t3.2xlarge`, EIP + **ALB** |
...
| **ChunkLoadError** `/_next/static/chunks/*.js` | (1) nginx `auth_request`→302 HTML §8.2 · (2) **2노드 빌드 불일치**→404 §8.2.1 |
...
### 8.2.1 ChunkLoadError — 2노드 ALB + **빌드 revision 불일치** (404)

**증상:** `/_next/static/chunks/*.js` **404 Not Found**. MIME type `text/html` (daemon SPA fallback 또는 nginx HTML). `ChunkLoadError` · CSS `text/html` 거부.

**원인 (2026-07 staging D6):** ALB가 **node1·node2를 round-robin**. HTML(`location /`)과 `/_next/`(`open_design_daemon_local`) 모두 **요청이 도착한 EC2의 로컬 daemon**만 본다.  
node2만 `deploy.sh` 했고 node1은 **이전 Docker 이미지**이면 — HTML은 A노드(청크 해시 X), 정적 요청은 B노드(청크 X 없음) → **404**.

nginx `auth_request` 문제(§8.2)와 다름 — curl 이 **404** 이면 이 절을 본다.

**확인 (각 EC2):**

```bash
CHUNK=/_next/static/chunks/02-m5p8d7iq_f.js   # DevTools 에서 실패한 경로
curl -sS -o /dev/null -w "node local: %{http_code}\n" "http://127.0.0.1:7456${CHUNK}"
docker inspect open-design-daemon --format '{{.Id}} {{.Created}}' 2>/dev/null || docker ps --format '{{.Names}} {{.Image}}'
cd ~/neural/ns-open-design && git rev-parse --short HEAD
```

한쪽만 **404** 또는 **git SHA / image ID 다름** → 빌드 skew.

**해결:**

1. **양쪽 EC2 동일 revision deploy** (node1 먼저, node2 다음 — 또는 `rolling_deploy.sh`):
   ```bash
   cd ~/neural/ns-open-design && git fetch && git checkout staging && git pull
   cd deploy/teamver && bash deploy.sh --staging --rds
   sudo bash devops/nginx/apply_teamver_design_staging_nginx_conf.sh
   ```
2. Mac에서 `rolling_deploy.sh` ([39_4 §3.5](./39_4_배포_Terraform_운영_Runbook.md#35-실행-예-staging-2노드)) — drain → deploy 순. **Mac에서 빌드 아님**, EC2 `deploy.sh` 가 compose build.
3. 브라우저 **강력 새로고침** 또는 시크릿 (이전 HTML 캐시와 새 청크 혼선 방지).
4. **배포 창 임시 완화:** ALB target stickiness ON (쿠키) — 근본 해결 아님; **양쪽 동일 SHA** 가 SSOT.

**예방:** Phase 4 운영 규칙 — `.env.staging`·**동일 git SHA**·동일 image tag on all nodes ([39_4 §2.3](./39_4_배포_Terraform_운영_Runbook.md#23-환경-동기화)).
