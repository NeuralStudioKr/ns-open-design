...
Main FE `getDesignDriveAssetLaunchUrl()`은 `teamverDriveAssetId`, `teamverDriveAssetName`, `teamverDriveAssetMimeType`을 Design URL에 전달한다. Design `driveLaunchHandoff.ts`가 이를 1회 읽고 browser history에서 제거한 뒤 composer의 Drive import modal을 연다. 대상 asset은 사전 선택되지만 자동 반입하지 않으며, 기존 파일 허용 정책과 사용자 확인을 통과해야 한다.
