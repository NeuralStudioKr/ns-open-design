...
| full Drive browser (최근/검색 결과/파일 grid) | ☐ Phase 1-2b |
...
**남음:** Main FE `DriveImportModal` 수준의 full browser(최근 파일, 자산 grid, breadcrumb). Server-side folder search는 loop 167에서 완료.
...
5. **API 클라이언트 추출**: `teamver/drive/driveApi.ts` — Main FE `services/drive.ts`의 핵심 함수만 포팅 (`fetchFolderTree`, `fetchDriveList`, `fetchHomeRecent`, `fetchHomeSearch`) — ☐.
6. **Full browser**: 좌측 폴더 트리 + 우측 자산 그리드 + breadcrumb — ☐.
...
| 2026-Q3 | Phase 1-2b (full Drive browser) | ☐ |
