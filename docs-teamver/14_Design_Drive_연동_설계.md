...
2. **Drive import UI**: composer modal/full browser와 Main Drive handoff 완료. 다중 파일 handoff와 import 원본으로 돌아가기 링크는 후속.
...
Main FE `getDesignDriveAssetLaunchUrl()`은 `teamverDriveAssetId`, `teamverDriveAssetName`, `teamverDriveAssetMimeType`을 Design URL에 전달한다. `teamverDriveIntent=create-slides`가 함께 오면 Design은 전체 Drive import browser 대신 **one-confirm modal**(`TeamverCanvasSlideLaunchModal`)을 연다. 사용자가 **슬라이드 만들기**를 누르면 design-api `import-drive`로 source를 가져온 뒤 slide 생성 prompt run을 즉시 시작한다(loop 191). intent 없는 handoff는 기존처럼 import modal만 열고 asset을 사전 선택한다.
...
7. **Full browser**: 최근 파일 + 자산 그리드형 publish destination helper — ☐.
