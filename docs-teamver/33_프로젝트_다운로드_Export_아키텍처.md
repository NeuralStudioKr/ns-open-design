...
| 포맷 | API | daemon 처리 |
|------|-----|-------------|
| HTML | `POST /export/html` | scratch HTML → headless snapshot (deck slide 펼침·nav 제거·resource inline). headless Chromium unavailable이면 deck flatten CSS를 주입한 static HTML로 degrade |
| ZIP | `POST /export/zip` | 위 snapshot을 `index.html` 로 ZIP 패키징. headless Chromium unavailable이면 static HTML fallback을 ZIP에 패키징 |
| PDF | `POST /export/pdf` | headless Chromium → PDF bytes |
| PNG/JPEG/WebP | `POST /export/image` | headless 캡처 (deck slide index 지원) |

#### 4.2.1 Export memo cache bypass — `fresh=1` (2026-07-03)

daemon export memo cache([34 §20](./34_Export_성능_개선_로드맵.md)) hit 시 Chromium render를 생략한다. WebGL 배경 rasterize 실패 등 **잘못된 snapshot이 캐시되면** 재다운로드해도 동일 artifact가 반복될 수 있다.

| 레이어 | 동작 |
|--------|------|
| **daemon** | `?fresh=1` 또는 JSON body `{ fresh: true }` → cache **lookup skip** · render 후 cache **populate** (갱신) |
| **FE** | 기본: cache 사용 · **Shift+click PDF** (embed Export 메뉴) → `fresh: true` |

**API 예:** `POST /api/projects/:id/export/pdf?fresh=1` body `{ deck, fileName, title, fresh: true }`

**코드:** `export-cache-runtime.ts` · `import-export-routes.ts` · `exports.ts` · `TeamverExportMenu.tsx`

**타임아웃 env:** `OD_EXPORT_TIMEOUT_MS` (default 30000, min 1000) — headless export per-job ceiling.
...
**HTML/ZIP 런타임 폴백 (2026-07-06):** staging/prod 컨테이너에서 `/usr/bin/chromium`이 즉시 `SIGTRAP`으로 종료되는 경우 `headless Chromium unavailable`이 발생할 수 있다. HTML/ZIP은 PDF/이미지와 달리 서버가 원본 HTML에 deck flatten CSS를 주입해 독립 실행형 파일로 degrade 할 수 있으므로, 이 경우 `/export/html`과 `/export/zip`은 500 대신 static fallback artifact를 내려준다. PDF/이미지는 실제 raster/PDF render가 필요하므로 Chromium launch 안정화(`--disable-crashpad`, `--no-zygote`, writable `/tmp/.chromium`)와 FE browser-print fallback 경로를 사용한다.
...
| 항목 | 결정 | 이유 |
|------|------|------|
| **이미지 GET** | 프리사인 우선 (`AuthenticatedProjectFileImage`) | 채팅 썸네일·파일 뷰어 미리보기 바이트가 데몬을 건너뜀. mint 404는 `/raw/` 재시도 안 함(이중 404 방지). |
...
| 날짜 | 내용 |
|------|------|
| 2026-07-06 | §4.2 HTML/ZIP `headless Chromium unavailable` static fallback · Chromium launch 옵션 보강 |
| 2026-07-03 | §4.2.1 export `fresh=1` cache bypass · Shift+click PDF · `OD_EXPORT_TIMEOUT_MS` |
