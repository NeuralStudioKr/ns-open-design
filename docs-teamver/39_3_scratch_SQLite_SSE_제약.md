...
#### Litestream 노드별 replica prefix

Phase 4 에서 두 EC2가 각자 `app.sqlite` 를 write 하므로 **replica prefix 를 노드별로 격리**해야 write 충돌·overwrite 를 방지할 수 있다.

| 계층 | 값 |
|------|----|
| `OD_NODE_ID` | EC2 instance-id (deploy.sh 가 IMDS 로 자동 주입) |
| `LITESTREAM_REPLICA_PATH` | `litestream/${sanitized-node-id}/app.sqlite` (deploy.sh 가 lower-kebab 로 sanitise) |
| Legacy (single node) | `litestream/app.sqlite` — 기존 데이터 이관 불필요 |

- `deploy.sh` 는 `OD_NODE_ID` 를 소문자·`[a-z0-9-]` 로 sanitise 하여 `LITESTREAM_REPLICA_PATH` 를 계산 → docker-compose 로 전달 → `litestream.yml` 이 `${LITESTREAM_REPLICA_PATH:-litestream/app.sqlite}` 로 치환.
- Restore: `bash scripts/restore_app_sqlite_from_s3.sh --production --litestream --replica-id <node-id>` — 특정 노드의 replica 만 복원. `--replica-path` 로 raw override 도 가능.
- Verify: `bash scripts/verify_litestream_replica.sh --production` 은 `LITESTREAM_REPLICA_PATH` 를 존중해 배너에 prefix 를 표시.
- 두 EC2 간 sqlite 메타 통합은 이 페이즈 밖 — **Postgres 전환(B5)** 로 해결.
...
| daemon node id | `OD_NODE_ID` → `/api/health.nodeId` · `X-OD-Node-Id` header (`server.ts`) |
| design-api node id | `TEAMVER_DESIGN_NODE_ID` → `/healthz.node_id` · `X-Design-Api-Node` header (`app/middleware/node_id.py`) |
| Litestream replica path | `LITESTREAM_REPLICA_PATH` — deploy.sh 자동 계산 (`litestream.yml`) |
