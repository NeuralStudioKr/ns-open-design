...
- ✅ daemon headless deck export base는 contracts `healDeckHtmlForStandaloneExport`를 사용해 HTML/ZIP/PPTX snapshot 준비 기준을 통일한다. PDF는 §0.5 이후 preview HTML 기준으로 분리한다.
...
| 2.1b-2 | S3 offload store skeleton (`HEAD`/`PUT`, fail-open result contract) | ✅ 완료 — route 미연결, 테스트 더블 검증 |
| 2.1b-3 | export route best-effort offload 연결 (PDF/image/PPTX/HTML/ZIP) | ✅ 완료 — body/filePath cache hit 모두 offload, 기존 stream fallback 유지 |
| 2.1b-4 | presigned GET URL builder (virtual-host/path-style, TTL, STS token) | ✅ 완료 — route 전환 전 URL 계약 테스트 |
| 2.1b-5 | presigned GET async 발급 함수 (`ready|disabled|failed`) | ✅ 완료 — route redirect 전환 전 fail-open 계약 |
| 2.1b | presigned GET 발급 API (session-gated) | ✅ 완료 — ticket GET에서 single-use claim 후 302, 실패 시 stream fallback |
...
- ✅ dedicated export worker process 1차 — `OD_EXPORT_DEDICATED_WORKER_ENABLED=1` + async job API에서 render 단계만 별도 Node child process로 실행해 daemon event loop/heap 영향과 Chromium crash 전파를 줄이는 프로세스 격리 기반 확보
- dedicated export worker container/queue — RDS/Redis/SQS 등 shared job store를 둔 별도 compose/ECS service consume 방식은 후속 Phase 3b
- ✅ “대형 deck은 PDF만” 정책 문구 polish — PPTX 제한 초과를 “오래 걸릴 수 있음”이 아니라 “N장 제한 초과, PDF 다운로드 권장”으로 명확히 안내

#### Phase 3a dedicated worker process 운영 메모

- 기본값은 `OD_EXPORT_DEDICATED_WORKER_ENABLED=0`이며 배포만으로 경로가 바뀌지 않는다.
- 켜려면 `OD_EXPORT_ASYNC_JOBS_ENABLED=1`, `VITE_TEAMVER_EXPORT_ASYNC_JOBS_ENABLED=1`, `OD_EXPORT_DEDICATED_WORKER_ENABLED=1`을 함께 검증한다. FE가 sync export를 쓰면 dedicated worker 경로를 타지 않는다.
- 이번 단계는 **별도 child process 격리**다. `docker compose ps`에 별도 `export-worker` 컨테이너가 보이는 구조는 아직 아니며, 완전 컨테이너 분리는 shared job store가 들어가는 Phase 3b에서 진행한다.
- 문제 발생 시 `OD_EXPORT_DEDICATED_WORKER_ENABLED=0`만 되돌리면 async job은 기존 daemon in-process runner로 fallback한다.
- worker child는 render/cache 산출물만 parent에 반환하고, download ticket 등록·S3 offload·job status update는 parent daemon이 계속 담당한다. 따라서 S3/IAM/offload 계약은 기존 route와 동일하게 유지된다.
...
| `OD_EXPORT_OFFLOAD_BUCKET` / `REGION` / `PREFIX` | staging bucket / `exports` | 필요 시 전용 exports bucket | export offload 대상 |
| `OD_EXPORT_OFFLOAD_PRESIGN_TTL_SEC` | `300` | `300` | presigned GET TTL (60~900 clamp) |
...
| `OD_EXPORT_OFFLOAD_BUCKET` / `REGION` / `PREFIX` | fallback `OD_S3_*` / prefix empty | 전용 export offload bucket/prefix |
| `OD_EXPORT_OFFLOAD_PRESIGN_TTL_SEC` | `300` | presigned GET TTL (60~900 clamp) |
...
| 날짜 | 내용 |
|------|------|
| 2026-08-25 | ✅ `feat/export-module-split` → `staging` merge 반영 — 최신 `origin/staging` 추가 커밋까지 재병합 후 export module split을 staging에 통합. round25 16:9 pin이 slide 본문을 `<div data-od-slide-flow>`로 감싸는 최신 snapshot 형태를 web export 테스트가 허용하도록 보정하고, 첫 browser-print fallback 케이스는 contracts dynamic import 비용을 감안해 timeout을 10s로 조정. 실제 `deploy/teamver/.env.staging`에도 `OD_EXPORT_ASYNC_JOBS_ENABLED=0`, `VITE_TEAMVER_EXPORT_ASYNC_JOBS_ENABLED=0`, `OD_EXPORT_PPTX_MAX_SLIDES=40` 명시. 검증: contracts build, web export tests 112 passed, daemon export suite 19 passed, async export route 4 passed |
...
| 2026-07-15 | Export offload presign 함수 추가 — credential provider 기반 `presignExportOffloadGet`이 `ready|disabled|failed`로 수렴하도록 해 route 302 전환 전 fail-open 계약을 고정 |
| 2026-07-15 | Export offload presigned GET URL builder 추가 — virtual-host/path-style endpoint, `OD_EXPORT_OFFLOAD_ENDPOINT`, STS token, TTL query 계약을 테스트로 고정 |
| 2026-07-15 | Export route best-effort offload 연결 — PDF/image/PPTX/HTML/ZIP export ticket 응답 직전 `putExportOffloadObject`를 fail-open으로 호출하고 `offloadStatus`를 ticket JSON에 포함. filePath-only cache hit는 메모리 재복사 없이 `skipped_source_file` 처리 |
| 2026-07-15 | Export S3 offload store skeleton 추가 — `statObjectAtKey`/`writeObjectAtKey` 기반 `disabled|hit|uploaded|failed` 결과 계약을 만들고, route 미연결 상태에서 테스트 더블로 fail-open 동작 검증 |
| 2026-07-15 | Export offload config parser 추가 — `OD_EXPORT_OFFLOAD_*` env, `OD_S3_*` fallback, presign TTL clamp(60~900)를 코드·테스트·문서로 고정 |
