...
**관련:** [04_구현_우선순위](./04_구현_우선순위.md) · **[09 저장소·격리](./09_Design_저장소_격리_출시게이트.md)** · [07_VM_배포_인프라](./07_VM_배포_인프라.md) · [01_통합_아키텍처](./01_통합_아키텍처.md) · [open-design-index](./open-design-index.md)
...
> **Prod 오픈 전** [09 저장소·격리](./09_Design_저장소_격리_출시게이트.md) Phase 0~3 필수. 커스텀 design-app FE·run/job wrapper BE는 **출시 이후** 단계.
...
| 영역 | 예시 | 경로 |
|------|------|------|
| 브랜딩 | 로고·타이틀·favicon "Teamver Design" | `apps/web` public, `layout.tsx` |
| 네비게이션 | OD 마케팅·Discord 링크 숨김 | `EntryView`, 설정 등 |
| 테마 (선택) | Teamver primary 색 CSS 변수 | `apps/web/src/styles` |
| 배포 | `deploy/teamver` env·이미지 태그 | 이미 존재 |
| 환경 | `OD_ALLOWED_ORIGINS`, `OD_API_TOKEN` | compose / K8s secret |

**넣지 않을 것:** `@teamver/app-sdk` import, Teamver JWT를 OD `/api/chat`에 전달, OD DB 스키마 변경.

패치는 가능하면 `patches/teamver/` 또는 소수 파일 + 주석으로 추적해 upstream merge 비용을 줄인다.
...
| 순서 | 항목 | 문서 |
|------|------|------|
| 1 | Drive export 자동 업로드 (G7) | [09 Phase 4](./09_Design_저장소_격리_출시게이트.md) · [03](./03_키_저장소_Drive_DB.md) |
| 2 | usage·과금 hook | `app-sdk` |
