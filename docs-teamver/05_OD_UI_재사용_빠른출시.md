...
[x] slide run SPA 백그라운드 지속 — 페이지/대화 이동 시 최초 SSE consumer 유지, 원 conversation 메시지·생성 파일 저장, 재진입 중복 구독 방지
[x] background run 전역 UX — 다중 프로젝트 펼침/개별 열기 + active→success/failure in-app/desktop 완료 알림
...
```text
[x] POST /api/v1/projects/{id}/publish (HTML+PDF, 201/207/502)
