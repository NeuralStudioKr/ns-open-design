...
**관련:** [12_embed_로컬UX_제거](./12_embed_로컬UX_제거_체크리스트.md) · [13_1 중국어 deck 비노출](./13_1_embed_중국어_deck_템플릿_비노출.md) · [14_Design_Drive_연동_설계](./14_Design_Drive_연동_설계.md) · [15_웹참조 BYOK FAQ](./15_웹참조_BYOK_web_fetch_FAQ.md) · [04_구현_우선순위](./04_구현_우선순위.md) · [10_세션·OD패치](./10_세션·OD패치_보강.md)
...
| **P2 유지·검토** | Claude ZIP import, Figma URL import(비노출), Skills(deck) — **Template 탭은 slide-only에서 숨김** (비-deck/`od-new-generation` 생성 방지) |
...
| S-5c | 중국어 deck 템플릿 비노출 | embed: preview·썸네일 중국어 중심 deck **15개 denylist + guizang family** — daemon `/api/design-templates`·`/api/plugins` 응답에서 제외 + web 이중 필터. `od.content_locale: zh-CN` 메타 + denylist + catalog guard | `OD_DESIGN_TEMPLATES_EXCLUDE_ZH_CN` + `embed-chinese-deck-policy` (contracts) | ✅ [13_1](./13_1_embed_중국어_deck_템플릿_비노출.md) |
...
| Sh-8 | `FileViewer` chrome **download** menu — PDF / Image / HTML / Markdown / ZIP / Save as template / **PPTX** | 사용자 OS 다운로드 | 낮음 (로컬) | **유지** | PPTX 항상 노출 (`isTeamverPptxExportEnabled` → always on; 옛 env kill-switch 무시) |
...
- [ ] Home hero create 탭 — Slide deck만 (template shortcut 숨김)
- [ ] New project — Deck 탭만
- [ ] 자유 입력 create → `example-simple-deck` (od-default 금지) + `kind=deck` + `skipDiscoveryBrief`
- [ ] SessionModeToggle(Docs/chat) 숨김 — design 고정
