...
| S-5c | 중국어 deck 템플릿 비노출 | embed: preview·썸네일 중국어 중심 deck **11개** — daemon `/api/design-templates`·`/api/plugins` 응답에서 제외 + web 이중 필터. `od.content_locale: zh-CN` 메타 + denylist | `OD_DESIGN_TEMPLATES_EXCLUDE_ZH_CN` + `embed-chinese-deck-policy` (contracts) | ✅ [13_1](./13_1_embed_중국어_deck_템플릿_비노출.md) |
