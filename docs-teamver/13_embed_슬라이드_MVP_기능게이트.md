...
| `hideAssistantThinkingDetails` | `true` | loop 437 — embed에서는 thinking block 자체를 비노출. prose leak은 `internalAgentMarkup`/contracts sanitizer + daemon `thinking_delta` 분리 |
