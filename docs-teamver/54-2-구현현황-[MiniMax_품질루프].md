...
| 아직 번호로 안 봄 | heal이 아직 빈 칸으로 분류하지 않는 다음 제목. 테스트 전용. 예: `기둥 Q` |
...
- **후보 (칸 번호 Q):** 모델이 빈 3열을 다음 칸 번호(`기둥 Q`)로 채우면 그때 빈 칸 번호에 넣는다. `스무 번째`는 아직 번호로 보지 않는 서수 픽스처. extra-copy·stub는 주제 단어 목록이 아님(루프259·265).
...
- **후보 C (3열 residual):** 칸 번호 `Q` 이후, 또는 비율 >2.05 sidebar를 건드리지 않는 별도 layout residual.
