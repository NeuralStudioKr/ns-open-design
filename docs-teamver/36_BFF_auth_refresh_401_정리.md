...
**2026-08-26 Phase 2 (Plan A):** 크로스앱 mismatch 선제 감지는 **Design BFF `GET /auth/session` 서버 `main_sso_status`** (Main HttpOnly 쿠키를 BE가 판정) + focus 시 session-first reconcile. Main 변경 없음. epoch 쿠키는 Plan C(보류). SSOT: [0825-N01-0](./0825-N01-0-CTO전달-[BFF_auth_완전해결].md) · [0825-N01-1 §12](./0825-N01-1-상위기획-[BFF_session-probe_refresh_401_완전해결].md) · [0825-N01-2 §10~17](./0825-N01-2-구현설계-[BFF_session-probe_refresh_401_완전해결].md).
...
| 일시 (KST) | 내용 |
|------------|------|
| 2026-08-26 15:45 | Phase 2 Plan A — session 서버 판정 · epoch Plan C 보류 |
| 2026-08-26 11:35 | Phase 2 통합 링크 (epoch · 45 Stage 1–3) — **개정됨** |
