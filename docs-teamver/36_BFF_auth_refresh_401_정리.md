...
- 후자는 refresh를 “너무 자주 호출해서”가 아니라, **형제 노드가 stale BFF cookie를 Set-Cookie로 덮어쓰는 경합**이 원인이다.
- SSOT: **[39_10 HA 세션쿠키 경합 해결](./39_10_HA_세션쿠키_경합_해결.md)** · [22 §3.2f](./22_Drive_인증_Usage_연동_검토.md)
- 운영 시 Network에 Drive `session_expired`가 보이면 본 문서의 FE refresh 억제 로직부터 건드리지 말고, 39_10 체크리스트(쿠키명·secret·미변경 Set-Cookie 생략)를 본다.
