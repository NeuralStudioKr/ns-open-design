...
|--------|--------|---------------|
| `teamver-design-{env}-data` bucket | `s3:ListBucket` | `design/`, `design/*`, `exports/`, `exports/*` (condition) |
| `…/design/*` objects | `GetObject`, `PutObject`, `DeleteObject` | tenant prefix 하위 전체 |
| `…/exports/*` objects | `GetObject`, `PutObject`, `DeleteObject` | export offload presigned-download cache |
| bucket + `litestream/*` | `GetBucketLocation`, List(`litestream/` prefix), Get/Put/Delete objects | Litestream replica (09 G2) |
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringLike": {
          "s3:prefix": ["design/", "design/*", "exports/", "exports/*"]
        }
      }
    },
    {
    ...
    {
      "Sid": "ReadWriteExportOffloadObjects",
      "Effect": "Allow",
      "Action": ["s3:GetObject", "s3:PutObject", "s3:DeleteObject"],
      "Resource": "arn:aws:s3:::teamver-design-prod-data/exports/*"
    },
    {
      "Sid": "LitestreamBucketLocation",
      "Effect": "Allow",
    ...
    },
    {
      ...
      "Condition": {
        "StringLike": {
          "s3:prefix": ["litestream/", "litestream/*"]
        }
      }
    },
    {
      "Sid": "LitestreamReplicaObjects",
      "Effect": "Allow",
      "Action": ["s3:GetObject", "s3:PutObject", "s3:DeleteObject"],
      "Resource": "arn:aws:s3:::teamver-design-prod-data/litestream/*"
    }
  ]
}
```
...
| export offload `S3 PUT exports/... → 403 Forbidden` | 현재 credential(static IAM user 또는 instance profile)에 `exports/*` object 권한 없음 | §3.1 scope를 해당 credential에 적용. staging `.env`에 `OD_S3_ACCESS_KEY_ID`가 있으면 role이 아니라 **IAM user** policy를 먼저 확인 |
| production validate fail (static key) | `.env` 에 `AWS_ACCESS_KEY_ID` | key 제거, instance profile만 |
| Litestream S3 fail, daemon OK | Litestream도 IMDS 의존; **`s3:GetBucketLocation` 누락** 흔함 | role에 `GetBucketLocation` + `litestream/*` (§3.1); `terraform apply` 또는 콘솔 inline patch |
| `readonly database` / `_litestream_seq` | compose `teamver_od_data:/data:ro` | `:ro` 제거 — Litestream은 app.sqlite에 메타 테이블 쓰기 필요 |
| `GetBucketLocation` AccessDenied | instance profile에 bucket-level action 없음 | `teamver-design-{env}-app` role에 `s3:GetBucketLocation` on `teamver-design-{env}-data` 추가 후 litestream 재기동 |
| 구 Docker 이미지만 사용 | 코드/IMDS 경로 구버전 | staging 소스 `docker compose build open-design-daemon` |

**로그:**

```bash
docker logs teamver-design-litestream 2>&1 | tail -30
docker logs teamver-open-design-daemon 2>&1 | grep -E 'storage_init|materialization|IMDS'
```

### 9.1 Litestream `GetBucketLocation` 즉시 패치 (terraform apply 전)

EC2 role `teamver-design-staging-app` 에 **추가 inline policy** (기존 terraform policy 와 병존):

```bash
aws iam put-role-policy \
  --role-name teamver-design-staging-app \
  --policy-name teamver-design-staging-litestream-bucket-location \
  --policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Sid": "LitestreamGetBucketLocation",
      "Effect": "Allow",
      "Action": "s3:GetBucketLocation",
      "Resource": "arn:aws:s3:::teamver-design-staging-data"
    }]
  }'
...
docker compose -f docker-compose.yml -f docker-compose.staging.yml \
  --env-file .env.staging up -d --force-recreate litestream
docker logs teamver-design-litestream --tail 20
```

정식 SSOT: `ns-teamver-devops/terraform/services/teamver-design/s3.tf` `terraform apply` (§4.4).

**staging static key 우회** (`OD_S3_ACCESS_KEY_ID` in `.env`): compose가 Litestream에도 동일 key를 전달한다. 이 경우 **IAM user** policy에도 `GetBucketLocation` + `litestream/*` 가 있어야 한다. instance profile 패치가 더 권장.
...
staging이 static key로 동작 중이면 export offload도 같은 IAM user credential로 S3에 PUT/HEAD/GET을 수행한다. 따라서 `teamver-design-staging-s3` 같은 임시 user policy에도 §3.1과 동일하게 `design/*`, `exports/*`, `litestream/*` scope를 반영해야 한다. instance profile에만 Terraform policy를 적용하고 `.env.staging`에 static key가 남아 있으면, daemon은 계속 user key를 우선 사용하므로 `exports/*` 403이 해결되지 않는다.
