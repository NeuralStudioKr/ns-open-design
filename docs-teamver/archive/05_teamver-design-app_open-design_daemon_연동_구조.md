...
# teamver-design-app ↔ open-design daemon 연동 구조
...
| 문서 | 내용 |
|------|------|
| [01_1_open-design_teamver통합방안_chatgpt.md](./01_1_open-design_teamver통합방안_chatgpt.md) | wrapper·sidecar, adapter |
| [03_open-design_데몬_통신_규격.md](./03_open-design_데몬_통신_규격.md) | HTTP/SSE, `/api/runs` |
| [04_open-design_키_저장소_Teamver연동_검토.md](./04_open-design_키_저장소_Teamver연동_검토.md) | 키·artifact·Drive |
...
| 이유 | 설명 |
|------|------|
| **엔진 분리** | open-design은 **디자인 실행 엔진**; teamver-design-app은 **사용자 경험·Teamver 연동** 을 갖는 앱. 코드 fork 없이 **HTTP** 로 붙인다 ([01_1](./01_1_open-design_teamver통합방안_chatgpt.md)). |
...
- **사용자 비동기**: FE는 design-app BE의 **run/job API** 만 호출(폴링·SSE·웹소켓 등). **브라우저가 OD `:7456`에 직접 run을 넣지 않는다.**
- **앱 BE ↔ OD 비동기(권장)**: `POST /api/runs` → **202 + `runId`** → `GET /api/runs/:id/events`(SSE) 또는 `GET /api/runs/:id` 폴링 → export → Drive ([03](./03_open-design_데몬_통신_규격.md)).
...
[01_1](./01_1_open-design_teamver통합방안_chatgpt.md) **sidecar**: 앱 컨테이너(FE+BE) + daemon + `OD_DATA_DIR` 볼륨.
...
| `OdDaemonClient` | health, projects, runs, export ([03](./03_open-design_데몬_통신_규격.md)) |
| `OdRunWorker` | SSE consume, timeout, cancel |
| `ArtifactPublisher` | export → Drive ([04](./04_open-design_키_저장소_Teamver연동_검토.md)) |
| `TeamverContext` | SDK bootstrap |

공개 도메인 API는 [01_1](./01_1_open-design_teamver통합방안_chatgpt.md) 의 `generateDesignArtifact(...)` / 앱 REST로 노출해도 된다.
...
- Multi-tenant auth/billing·공정 큐는 OD 비목표 ([04](./04_open-design_키_저장소_Teamver연동_검토.md)).
