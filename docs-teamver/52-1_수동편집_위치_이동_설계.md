...
idle
  ...
  ▼
moving  ── Escape / cancel ──► idle (preview restore, no flush)
  │ pointermove → preview left/top
  │ pointerup
  ▼
commit (flush set-style 1회, history 1, remasure) → idle
```

`manualEditResizeSessionActiveRef`를 **`manualEditGeometrySessionActiveRef`** 로 일반화하거나, move도 동일 ref로 autosave pause를 공유한다 (이름 리네임은 구현 시 선택; 동작은 동일).
...
- [ ] absolute 카드 본체 드래그 → 미리보기·소스 left/top 갱신
- [ ] 핸들 드래그는 여전히 resize만
- [ ] static 문단은 grab 커서/이동 없음
- [ ] zoom 75% 정렬
- [ ] Esc 취소
- [ ] undo 1스텝 복귀
- [ ] Draw·text-edit 중 비활성
