...
- `end_turn` mid-`<script>` 잘림: chat scrub은 bare `document.addEventListener('keydown', e=>` / `ArrowDown` / `touchstart|touchend|wheel` 조각도 open-form으로 잘라낸다. preview는 `repairArtifactDocumentHead` → `stripTrailingUnclosedRawBlocks`가 미닫힘 trailing `<script|style>`를 제거하고 salvage `</body></html>`는 보존한다(슬라이드 유지 → stable 통과).
