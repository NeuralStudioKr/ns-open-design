...
- [x] 빈 입력 게이트 — 프롬프트 또는 첨부 없이 Next/생성 불가

## 진행 메모

- 2026-08-24: Drive import 모달 선택 칩에 New Slide와 같은 thumb/타입 아이콘 (`TeamverAttachChipVisual` 공유). selected 이미지도 thumb batch 대상에 포함.
- 2026-08-21: 첨부 칩에 이미지 썸네일 / 파일 타입 아이콘 / 확장자 유지 파일명. object URL은 File 객체 키로 안정화(중간 칩 제거 시 재발급 방지). thumb onError → 아이콘 폴백. 제거 버튼 aria에 파일명 포함. Drive 공유 드라이브 이미지는 `sharedDriveId`를 staging에 유지해 cache miss 시에도 thumb refetch. 파일 칩 React key에서 index 제거.
- 2026-08-19: 장수 상한 1–15. 첫 fill 안정성 캡(6–8)은 유지. 닫힌 덱이 요청보다 짧으면 숨김 top-up이 기존 장을 유지한 채 최대 2회·회당 약 6장 이어 붙임. 댓글 스코프·incomplete auto-continue와는 분리.
- 2026-08-18: 길이 칩에 장수(5–6 / 8–10 / 12–15) + Home 직접 지정. 자동 칩 유지. 프롬프트 `N장`이 최우선. 첨부→프롬프트→퀵 재배치는 보류.
- 2026-08-18: 빈 입력 게이트 — textarea·첨부 둘 다 없으면 Next/템플릿 스텝/Confirm 불가. 퀵설정만으로는 진행하지 않음. 안내는 푸터에 「무엇을 만들지 적어 주시거나, 참고 자료를 첨부해 주세요.」
