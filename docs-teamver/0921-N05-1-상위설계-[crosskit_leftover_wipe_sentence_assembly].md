# 0921-N05-1 상위설계 · Cross-kit leftover wipe / 문장 짜맞추기

상위: [0921-N04-1 AI fill 강제 · 서버 문장 짜맞추기 제거](./0921-N04-1-상위설계-[ai_fill_강제·짜맞추기_제거].md)  
선행: [0921-N04 kit pack Teamver-scope guard](./0921-N04-1-상위설계-[kit_pack_teamver_scope_guard].md)  
SSOT: [60](./60_Canvas_Slide_시스템_프롬프트_템플릿적용_개선.md) §1.46  
정책: persist / pad / continue / head-banner **불변**. 0918-N05 · 0921-N03 healer/fill · 0921-N04 Teamver-scope **삭제·되돌리지 않음**.
...
N04 kit-scope guard 는 Teamver 브리프에서 no-op 가 맞다. 이번 회귀는 **다른 킷 filler 가 Block Frame 을 오염**하고, **서버 leftover wipe/title fallback 이 MiniMax 문장을 자른 뒤 접미를 붙이는** 문제다.
...
| `dedupeIdenticalOutlineTitles` / pad | `${cover} · N` salt. 0921-N04 AI fill 상위설계가 금지. |
...
4. **pad/dedupe salt 금지.** `${cover} · N` 대신 role heading 을 쓰고, 그래도 겹치면 원제를 유지한다.
