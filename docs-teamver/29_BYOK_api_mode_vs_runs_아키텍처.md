...
### 4b. GET `/api/runs` 인증 경로 (nginx vs BFF)

BYOK embed 에서 GET 은 **빈 JSON** 이지만 nginx 는 여전히 **로그인 게이트**를 탄다.

```mermaid
sequenceDiagram
  participant FE as FE (embed)
  participant NGX as nginx stg-design
  participant Main as Main BE session-check
  participant BFF as design-api BFF
  participant OD as daemon :7456

  FE->>NGX: GET /teamver-bff/auth/session
  NGX->>BFF: proxy (no auth_request)
  BFF-->>FE: 200 authenticated

  FE->>NGX: GET /api/runs (background poll)
  NGX->>Main: auth_request session-check
  alt cookie missing / invalid
    NGX-->>FE: 302 stg.teamver.com/auth/signin
  else OK
    NGX->>OD: proxy + OD_API_TOKEN
    OD-->>FE: 200 runs:[]
  end
```
...
| 대응 | 상태 |
|------|------|
| FE `fetchTeamverDaemon` + `credentials: include` (`36f51072a`) | ✅ 코드 |
| staging web 재배포 + Network 검증 | ☐ |
| nginx `/api/*` → 401 JSON (302 제거) | ☐ P2 |
| BYOK 에서 GET poll 끄기 ([§9 Q3](#q3-get-apiruns-를-byok-에서-끄면-되나)) | 선택 P3 |
