...
**Stepper:** entry=template이면 내용 화면에서도 `(2) 템플릿`을 `complete`로 표시. `(2)` 클릭 시에만 템플릿 패널. 스텝·다음 CTA 카피는 모두 「템플릿」. 내용 스텝 푸터에 변경 칩을 두지 않음(61 안 B). 갤러리 진입에서 스텝2 카드를 고르면 내용 + ✓로 복귀. 명시 픽 제목은 스텝퍼에 표시.
