...
**갱신:** 2026-06-26 (loop 434 daemon strict S3 sync-up retry 반영)
...
| P1-10 | sync-up 실패 알람·재시도 (run 종료 후/strict registry sync) | `apps/daemon` + ops | 🟡 파일 단위 retry 3x + strict 작업 전체 retry(loop 434) + lazy/run-end `od_s3_sync_up_failed` JSON 마커 ✅ · **CloudWatch alarm EC2 apply ☐** |
