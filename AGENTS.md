...
These notes cover non-obvious runtime caveats for the Cursor Cloud VM. The startup/update script already runs `pnpm install` (which builds all workspace `dist/` outputs and rebuilds the `better-sqlite3` native addon via `scripts/postinstall.mjs`), so this section is only about running/testing, not installing.

- **Node runtime.** The base image's default `node` on `PATH` (`/exec-daemon/node`) is Node 22, but this repo requires Node `~24`. Node 24 is installed via `nvm` and prioritized ahead of the inherited Node 22 through a line in `~/.bashrc`; `pnpm` is provided by Corepack from that same Node 24 install. New interactive shells get Node 24 + `pnpm@10.33.2` automatically — just run `node`/`pnpm` normally. If `node --version` ever shows v22, prepend the nvm Node 24 bin dir to `PATH`.
- **Standard commands live elsewhere.** Lifecycle (`pnpm tools-dev …`), validation (`pnpm guard`, `pnpm typecheck`), and package-scoped test/build commands are documented in the "Common commands" section above and in each app's `AGENTS.md`; don't duplicate them here. `pnpm guard` and `pnpm typecheck` currently report some pre-existing violations unrelated to environment setup (residual `.mjs` files, non-exact `pg` dependency specs, a cross-app test import, an extra `e2e` `test:ui:teamver` script, and a `packages/contracts` test importing a renamed symbol) — the toolchains themselves run fine.
- **Web UI runs in Teamver embed mode on localhost.** `pnpm tools-dev run web` starts Next.js in dev (`NODE_ENV=development`), which makes `isTeamverEmbedMode()` true for `localhost`/`127.0.0.1`. In embed mode the web app polls a Teamver auth BFF that Next proxies to port 16000 (`BE_PORT`, see `apps/web/next.config.ts`) — a backend that `tools-dev` does NOT start. The home page renders, but submitting a brief from the browser is gated behind Teamver sign-in. Embed mode is off for any non-localhost hostname, but Next 16's dev cross-origin protection then blocks arbitrary hosts (needs `allowedDevOrigins`). To exercise the full brief→artifact loop without standing up the Teamver backend, drive the daemon directly: `POST /api/runs` (or the `od` CLI) with `{projectId, agentId, model, message}` — the daemon auto-binds the project's default conversation and streams the run.
- **Agent detection is slow here.** `GET /api/agents` can take ~60s because the VM has many real agent CLIs on `PATH` (e.g. `~/.opencode/bin`, `~/.kimi-code/bin`) and each model-listing/version probe times out. For fast, deterministic agent detection, start the daemon with a restricted `PATH` that excludes those agent dirs.
- **Offline agent runs (no API keys/login).** The `mocks/` replay CLIs drive a full agent run with zero tokens. Fetch recordings once (`bash mocks/scripts/fetch-recordings.sh --agent claude`), expose a single `claude` wrapper (`exec node "$PWD/mocks/mock-agent.mjs" --as claude "$@"`) on the daemon's `PATH`, and set `OD_MOCKS_TRACE=<8-char-id>` + `OD_MOCKS_NO_DELAY=1`. The daemon then detects `claude` and replays the recording as a real run. Note: anonymized recordings can contain truncated artifact HTML, so a replayed run proves the orchestration loop but may not produce a fully renderable artifact file.
