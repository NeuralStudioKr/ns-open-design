...
Standard commands live in `QUICKSTART.md` and the "Common commands" section above; this section only records non-obvious cloud-VM caveats.

- **Node 24 is required but a system Node 22 can shadow it.** The VM ships a `node` on `PATH` (under `/exec-daemon`) that is Node 22 and takes precedence over nvm's Node 24 in non-login shells. Node 24 (installed via nvm) plus Corepack `pnpm@10.33.2` is the working toolchain. If a command reports the wrong Node version or an engines error, prepend the nvm Node 24 `bin` dir to `PATH` for that shell (interactive login shells already pick it up via `~/.bashrc`). The startup update script runs `pnpm install` under Node 24.
- **`node-pty` native build is intentionally skipped** (it is not in `package.json#pnpm.onlyBuiltDependencies`). The daemon boots fine without it; only the in-app terminal feature degrades. The `pnpm install` warning about ignored `node-pty` build scripts is expected — do not add it to build approvals.
- **Running the product:** `pnpm tools-dev run web --daemon-port <p> --web-port <p>` (foreground) starts daemon + web. The daemon exposes `GET /api/health`. The Next.js dev web server compiles routes on first request, so the initial page load and first `/api/*` proxy call can take ~10-20s before returning 200 — this is normal, not a hang.
- **No agent CLIs or LLM/BYOK keys are present by default**, so real model calls do not work. To exercise the full chat → agent → rendered-artifact pipeline offline with zero tokens, use the replay harness in `mocks/` (see `mocks/README.md`): `bash mocks/scripts/fetch-recordings.sh` then run the daemon with a mock CLI on `PATH` plus `OD_MOCKS_TRACE=<id>` and `OD_MOCKS_NO_DELAY=1`.
- **Gotcha — do not put the whole `mocks/bin` on the daemon's `PATH`.** The ACP mock binaries (`devin`, `hermes`, `kimi`, `kiro`, `vibe`, `vela`) block on stdin waiting for a JSON-RPC handshake, so runtime detection (`GET /api/agents`) hangs while probing them. Expose only the specific mock you need. For a rendered preview without real file writes, use a `plain`-format runtime (e.g. a `qwen` wrapper that execs `node mocks/mock-agent.mjs --as qwen`) pinned to a trace whose `report` contains a complete `<artifact>` HTML block (mocks never write to disk, so filesystem runtimes like claude will not populate project files).
